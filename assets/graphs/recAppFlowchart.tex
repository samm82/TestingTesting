...
\begin{figure}
    \adjustbox{scale=0.89}{
        \begin{tikzpicture}[node distance=2cm,line width=1.25]
            ...
            \node (catapp)   [decisionM, below of=start,  yshift=-0.5cm] {Present Category is ``Approach''?};
            ...
            \draw [arrow] (catapp)   -- node[anchor=south] {\quad yes} (catreplace);
            ...
            \draw [arrow] (datagiven) -- node[anchor=west] {\enspace somewhat} (datasomematch);

            \draw [arrow] (datamatch)     -- (end.west);
            \draw [arrow] (datanomatch)   -- (end);
            \draw [arrow] (datasomematch) -- (end.east);
