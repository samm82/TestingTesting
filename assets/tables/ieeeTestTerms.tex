...
\newcommand{\techniqueCell}{\makecell{(Design)\footnote{``Test design
            technique'' is sometimes abbreviated to ``test technique''
            \ifnotpaper
                (\citealp[p.~11]{IEEE2022}; \citealpISTQB{})\else
                \cite{ISTQB}, \cite[p.~11]{IEEE2022}\fi; both terms refer to
            the technique used to design the test case(s).
        }\\Technique}}
\newcommand{\levelCell}{\makecell{Level\footnote{\procLevel{\citep}.}\\
        ...
        or ``Stage''\footnote{Used by \ifnotpaper
                (\citealpISTQB{}; \citealp[pp.~438-440]{vanVliet2000};
                implied by \citealp[p.~9]{Gerrard2000a}) and inconsistently
                by \citep[pp.~5\=/6 to 5\=/7]{SWEBOK2024}
                (see \discrepref{stage-level-syns})%
            \else
                \cite{ISTQB}, \cite[pp.~438-440]{vanVliet2000}%
            \fi.
        })}}
