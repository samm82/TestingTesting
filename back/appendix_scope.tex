...
\begin{itemize}
    ...
    \item \acf{emsec} testing \ifnotpaper
          ...
          emanation'' (p.~95), is also out of scope.
    \item Continuity testing is a child of non-functional testing
          \citep[p.~6\=/8]{SWEBOK2025} and a test type that is concerned with
          ``backup, restore and/or failover facilities''
          \citep[p.~50]{KoomenEtAl2006} which have corresponding test
          approaches. This form of continuity testing is in scope, but the child
          of hardware testing given by \citet[p.~21]{Firesmith2015} is not.
    ...
    \item Since control systems often have a software \emph{and} hardware
          ...
          \begin{itemize}
              \item A related note: ``path coverage'' or ``path testing''
                    ...
                    (\citealp[p.~5\=/13]{SWEBOK2025}; \citealp[p.~4]{Reid1996};
                    implied by \citealpISTQB{}) or through a model, such as
...
\acf{vnv} performed on the code \emph{itself}\---the \acf{sut}\---is in scope.
Therefore, we exclude the following test approaches either in full or in part:
...
\begin{itemize}
    ...
    \item Fault injection testing, where ``faults are artificially
          introduced\textsuperscript{\ref{add-fault}} into the \acs{sut}'', can
          be used to evaluate the effectiveness of a
          test suite \citep[p.~5\=/18]{SWEBOK2025}, which is a part of \acs{vnv}
    ...
    \item ``Mutation [t]esting was originally conceived as a
          ...
          modified version of the \acs{sut}'' \citep[p.~5\=/14]{SWEBOK2025},
    ...
    \item \phantomsection{}\label{nontech-req-test}
          ...
          (p.~293). %\citep[p.~293]{IEEE2017}
\end{itemize}

\newpage
\section{Static Testing}\label{static-test}
Throughout the literature, static testing is more ambiguous than dynamic
testing, with more ad hoc processes and inconsistent inclusion/exclusion from
the scope of software testing in general (see \flawref{static-test-flaw}).
Furthermore, it seems less relevant to our original goal of automatic
generating test cases. In particular, many
...
and parts of these static approaches may even be generated in the
future! For these reasons, we keep static testing in scope for this stage
of our research.
% , even though static testing will likely be removed at a later
% stage of analysis\thesisissuenote{41,44} based on our original motivation.
...
broadly than just to testing. The following terms are too vague to merit
tracking in our glossaries or analyzing further\thesisissuenote{39,44,28}:
\begin{itemize}
...
other languages. These are therefore considered out-of-scope, % \thesisissuetodo{63}
including the following non-exhaustive list of examples:

\begin{itemize}
    \item \acs{sql} injection is just defined as ``a type of code injection in the
          \acf{sql}'' \citepISTQB{}, which does not provide any new information.
    \item Similarly, \acs{sql} statement coverage \citep[Tab.~13]{DoğanEtAl2014}%
          \todo{OG Alalfi et al., 2010} is just statement coverage
          used specifically for \acs{sql} statements.
    ...
    \item Testing for ``faults specific to PHP'' is just a subcategory of
          ...
          are not exclusive to PHP.  % Remove recursive/unhelpful PHP acronym
...
\begin{enumerate}
    \item Black box conformance testing \citep[p.~25]{JardEtAl1999}
          ...
          % Conformance: Type (implied by its use of quality requirements (PMBOK, 2013, p. 557))
    \item Black-box integration testing \citep[pp.~345\==346]{SakamotoEtAl2013}
          % Specification-based: Technique (IEEE, 2022, p. 22; 2021, p. 8; Washizaki, 2024, p. 5-10; Hamburg and Mogyorodi, 2024; Souza et al., 2017, p. 3; Firesmith, 2015, pp. 46-47; Sakamoto et al., 2013, p. 344; implied by IEEE, 2022, pp. 2-4, 6-9)
          % Integration: Level (IEEE, 2022, pp. 12, 20-22, 26-27; 2021a, pp. 6, 11; 2021b, Fig. 2, pp. 41, 43, 51; 2021c, p. 6; Washizaki, 2025, p. 5-7; Hamburg and Mogyorodi, 2024; Lyu, 1996, p. 532; Black, 2009, pp. 12, 14, 24, 178; Peters and Pedrycz, 2000, Tab. 12.3; van Vliet, 2000, p. 438; Souza et al., 2017, p. 3; Sakamoto et al., 2013, p. 343; Barbosa et al., 2006, p. 3)
    ...
    \item Checklist-based reviews \citepISTQB{}
          ...
          % Reviews: Approach
    \item Closed-loop HiL\footnote{See \flawref{hil-acro}.} verification \citep[p.~6]{PreußeEtAl2012}
    ...
    \item Conformity evaluations \citep[p.~93]{IEEE2017}
          % Conformance: (inferred from ``nonconformity'' as an antonym of ``conformance'' (IEEE, 2017, p. 292); implied by its use of quality requirements (PMBOK, 2013, p. 557))
          % Evaluations: Out of Scope (not useful; see \Cref{vague-terms})
    \item Design-based systems testing \citep[pp.~138, 281]{Hetzel1988}
          % Design-based: Technique?
          % Systems: Level (Black, 2009, p. 14; Hetzel, 1988, pp. 122, 134-139, 149)
    \item Elastic end-to-end testing \citep[p.~93:30]{BertolinoEtAl2019}
          % Elastic: Type (implied by its quality (Bertolino et al., 2019, p. 93:30))
    ...
    \item Formal reviews (\citealpISTQB{}; \citealp[p.~12\=/14]{SWEBOK2025})
    ...
    \item Incremental integration testing \citep[pp.~601, 603, 605\==606]{SharmaEtAl2021}\todo{OG [19]}
          ...
          % Integration: Level (IEEE, 2022, pp. 12, 20-22, 26-27; 2021a, pp. 6, 11; 2021b, Fig. 2, pp. 41, 43, 51; 2021c, p. 6; Washizaki, 2025, p. 5-7; Hamburg and Mogyorodi, 2024; Lyu, 1996, p. 532; Black, 2009, pp. 12, 14, 24, 178; Peters and Pedrycz, 2000, Tab. 12.3; van Vliet, 2000, p. 438; Souza et al., 2017, p. 3; Sakamoto et al., 2013, p. 343; Barbosa et al., 2006, p. 3)
          %           OR Technique (implied by Sharma et al., 2021, pp. 601, 603, 605-606)
    \item Informal reviews (\citealpISTQB{}; \citealp[p.~12\=/14]{SWEBOK2025})
    ...
    \item Legacy system integration (testing) \citep[Tab.~2]{Gerrard2000a}  %\qtodo{Is this orthogonal? Investigate legacy testing}
          % Legacy: Approach
    ...
    \item Online \acs{mbt} \citepISTQB{}
          ...
          % Model-based: Practice (IEEE, 2022, p. 22; 2021, p. viii)
          %           OR Technique (Engström and Petersen, 2015, pp. 1-2; Kam, 2008, p. 4; implied by IEEE, 2022, p. 32; 2021, p. 7; 2017, p. 469)
    \item Requirements-based systems testing \citep[p.~135]{Hetzel1988}
          % Requirements-based: Technique (IEEE, 2022, pp. 8, 22; 2021c, pp. 5, 22-23, Fig. 2; implied by Firesmith, 2015, p. 47)   
          % Systems: Level (Black, 2009, p. 14; Hetzel, 1988, pp. 122, 134-139, 149)
    \item Role-based reviews \citepISTQB{}  % \qtodo{Is this orthogonal? Investigate role-based testing}
          % Role-based: Practice?
          % Reviews: Approach
    \item Scenario walkthroughs \citep[Fig.~4]{Gerrard2000a}  % \qtodo{Is this orthogonal? Investigate scenario-based testing}
    ...
    \item Scenario-based reviews \citepISTQB{}  % \qtodo{Is this orthogonal? Investigate scenario-based testing}
