...
\def\exAutoFlaws{\multiSynCount{} terms used as synonyms to two (or more)
    disjoint test approaches and \parSynCount{} pairs of test approaches that
    may either be synonyms or have a parent-child relationship.}
...
\NewDocumentCommand\contraSummary{s}{\IfBooleanTF#1{contradictions are}
    {Contradictions are by \emph{far}} the most common manifestation. This is
    likely because these are the most obvious flaws to detect automatically
    (see \Cref{\ifnotpaper auto-flaw-detect,\fi flawDmns}) and because two
    (groups of) authors using different resources and not communicating have a
    high chance of disagreeing.}
