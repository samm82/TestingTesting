...
% Examples of flaws
\def\genDefectFlaw{``defect'' may be used as a ``generic term that can refer to
    either a fault (cause) or a failure (effect)'' \ifnotpaper
        (\citealp[p.~124]{IEEE2017}; \citeyear[p.~96]{IEEE2010}\todo{OG 2005})%
    \else \cite[p.~96]{IEEE2010}, \cite[p.~124]{IEEE2017}\fi}
...
\NewDocumentCommand{\contraBothWrong}{s}{are considered contradictions and not
    \IfBooleanTF#1{mistakes}{\nameref{wrong}}, since this would require
...
\def\redunsNote{Section omitted for brevity.}
% Assisted by GitHub Copilot
\NewDocumentCommand{\reduns}{s o}{\ifnotpaper\nameref{redun}\else%
        Redundancies\IfBooleanTF{#1}{\TblrNote{#2}}{\footnote{\redunsNote}}\fi}
