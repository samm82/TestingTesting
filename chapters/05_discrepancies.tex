...
After gathering all these data\footnote{Available in \texttt{ApproachGlossary.csv},
    \texttt{QualityGlossary.csv}, and \texttt{SuppGlossary.csv} at \ifblind{
        ...
        \url{https://github.com/samm82/TestGen-Thesis}}.}, we find many
discrepancies. To better understand and analyze them, we group them by their
syntax and their semantics. Syntactic discrepancies (\Cref{sntxDiscreps})
...
other hand, semantic discrepancies (\Cref{smntcDiscreps}) describe the
...
definition disrepancies (\Cref{defs}). Within these sections, \ifnotpaper
...
shown in \Cref{tab:sntxDiscreps,tab:smntcDiscreps}, respectively, where a given
...
discrepancies listed in \Cref{sntxDiscreps,smntcDiscreps} are not equal,
...
semantics of discrepancies listed above\ifnotpaper\ (\Cref{aug-discrep-analysis}
    outlines how this is done)\fi, causing a further mismatch between the counts
in \Cref{tab:sntxDiscreps,tab:smntcDiscreps} and the counts in \Cref{sntxDiscreps,%
    smntcDiscreps}. The ``problem'' subsets of testing include
...
used consistently\ifnotpaper\ (see \Cref{tab:otherTestTerms})\fi. The most
...
terms that are not synonyms themselves. \ifnotpaper Sometimes, these terms
    ...
    \begin{figure}[hbtp!]
        ...
        \begin{tikzpicture}
            ...
            \draw[thick] (st)  -- (uct) node [midway, align=center, left=12pt] {\citealpISTQB{};\\ \citealp[pp.~47--49]{Kam2008}\\ (see \discrepref{use-case-scenario})\\};
...
\input{build/SmntcDiscrepPars} \else; for example, performance \fi testing and
...
their sources in \Cref{tab:sntxDiscreps,tab:smntcDiscreps}. \ifnotpaper The following
...
equivalent to exhaustively testing the program''\footnote{The contradictory
    definitions of path testing given in \discrepref{path-test} add
    another layer of complexity to this claim.}, this overlooks the effects of
input data \ifnotpaper
