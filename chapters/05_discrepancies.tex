...
\ifnotpaper \input{assets/graphs/pieCharts} \fi
...
\begin{enumerate}
    \item % Discrep count (SYNS): {ISO_IEC2023b} {IEEE2017} | implied by {BaresiAndPezzè2006}
          ...
          may be necessary.
          \ifnotpaper
    \item % Discrep count (SYNS): {IEEE2017} {IEEE2013} | {IEEE2022} {IEEE2017} {Perry2006}
          ...
          \fi
    \item % Discrep count (SYNS): {PetersAndPedrycz2000}
          \ifnotpaper \citeauthor{PetersAndPedrycz2000} claim \else Reference
              \cite{PetersAndPedrycz2000} claims \fi that ``structural testing
    ...
    \item % Discrep count (SYNS): ISTQB | {Patton2006}
          % TODO: NEAR SYNS?
          \ifnotpaper \citetISTQB{} claim \else Reference \cite{ISTQB}
              claims \fi that code inspections are related to peer reviews but
          \ifnotpaper\else reference \fi \citet[pp.~94-95]{Patton2006} makes
          them quite distinct.
    \item % Discrep count (SYNS): ISTQB | {PetersAndPedrycz2000}
          ...
          as synonyms by \ifnotpaper\else reference \fi \citetISTQB{} but
          \ifnotpaper \citet[p.~484]{PetersAndPedrycz2000} imply
          \else reference \cite[p.~484]{PetersAndPedrycz2000} implies \fi that
          they are different, saying a more structured walkthrough may have
          specific roles.
          % TODO: Formalize; are the following legit discrepancies? Are they inferred?
          \ifnotpaper
    \item \ifnotpaper\citet[p.~18]{SneedAndGöschl2000}\todo{OG Hetzel88} give
          \else Reference \cite[p.~18]{SneedAndGöschl2000} gives \fi ``white-'',
...
Parent relations are also not immune to difficulties\ifnotpaper, as shown by
the following discrepancies:
\begin{enumerate}
    \item % Discrep count (PARS): {ISO_IEC2023a} | {Firesmith2015}
          Performance \else; for example, performance \fi and security testing
          are given as subtypes of reliability testing by \ifnotpaper\else
              reference \fi \citep{ISO_IEC2023a}, but these are all listed
          separately by \ifnotpaper\else reference \fi
          \citep[p.~53]{Firesmith2015}. \ifnotpaper
...
\end{enumerate}\fi\phantomsection{}\label{selfPars}
...
\input{chapters/05e_cat_discreps}
...
\input{chapters/05a_std_discreps}
...
\input{chapters/05b_meta_discreps}

\ifnotpaper\subsubsection{Other Discrepancies from \texts{}}
    \input{chapters/05c_text_discreps}\fi

\subsubsection{Other Discrepancies from \papers{}}
\input{chapters/05d_paper_discreps}
