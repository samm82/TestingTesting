...
also given, although not listed separately in \refDiscrepsReqsTable{}; these
...
\ifnotpaper
      ...
      \label{oat-discrep}
      % Discrep count (OTHER): {IEEE2022} ISTQB | {SWEBOK2024} {ISO_IEC2018} {IEEE2017} {SWEBOK2014}
      % Severity: Med (Std, Meta)
...
\begin{itemize}
      ...
      \item \textbf{Disaster/Recovery Testing} serves to evaluate if a system
            ...
            % Discrep count (OTHER): {IEEE2017} | {IEEE2021}
            % Severity: High (Std)
      ...
      \item \textbf{Backup/Recovery Testing:} Testing that determines the
            ...
            \citetext{p.~37}. % Discrep count (OTHER): {IEEE2021} | {IEEE2021}
            % Severity: Med (Std)
            It also seems to overlap with ``backup and
            recovery testing'', which adds confusion.
            % Discrep count (OTHER): {IEEE2021} | {IEEE2013}
            % Severity: High (Std)
      ...
      \item \textbf{Failover Testing:} Testing that ``validates the SUT's
            ...
            % Discrep count (OTHER): {IEEE2021} | {Firesmith2015}
            % Severity: Med (Meta)
\end{itemize}
...
\begin{enumerate}
      \item % Discrep count (OTHER): {IEEE2021} | {Firesmith2015} {Bas2024}
            % Severity: Med (Meta, Other)
            ...
            \fi
      \item % Discrep count (OTHER): {IEEE2021} | implied by {ISO_IEC2023a}
            % Severity: Med (Meta, Other)
            \ifnotpaper \citeauthor{IEEE2021} \else ISO/IEC and IEEE \fi give
      ...
      \item % Discrep count (OTHER): {SWEBOK2024}
            % Severity: High (Meta)
...
\input{chapters/05d_other_discreps}
