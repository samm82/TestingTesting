...
\nameref{syns} and \nameref{pars}. As an example, \tourDiscrep*{}, which is a
case of contradictory definitions; therefore, it is included with the
\nameref{contra} (its class) \emph{and} with the \nameref{defs} (its category).
...
source category (see \Cref{sources}).
...
source category (see \Cref{sources}) and/or with a ``more trusted'' one
(i.e., a previous row in the table). The numbers of (Exp)licit and (Imp)licit
(see \Cref{rigidity}) discrepancies are also presented in these tables. Since
...
discrepancies listed in \Cref{discrepClasses,discrepCategories} are not equal,
as those automatically uncovered based on semantics \ifnotpaper
    (see \Cref{auto-discrep-analysis}) \fi are only listed in the corresponding
category section for clarity; they still contribute to the counts in
\Cref{tab:discrepClss}!
...
categories of discrepancies listed above\ifnotpaper\ (\Cref{aug-discrep-analysis}
    outlines how this is done)\fi, causing a further mismatch between the counts
in \Cref{tab:discrepClss,tab:discrepCats} and the counts in \Cref{discrepClasses,%
    discrepCategories}. The ``problem'' subsets of testing include
...
their sources in \Cref{tab:discrepClss,tab:discrepCats}. \ifnotpaper The following
...
analysis of the generated graphs, \ifnotpaper which are \else with the most
    prominent \fi given in \Cref{tab:parSyns}. Of particular note is the
relation between path testing and exhaustive testing. While
\citet[p.~421]{vanVliet2000} claims that path testing done completely ``is
equivalent to exhaustively testing the program''\footnote{The contradictory
    definitions of path testing given in \discrepref{path-test-discrep} add
    another layer of complexity to this claim.}, this overlooks the effect of
implementation issues \citep[p.~476]{PetersAndPedrycz2000} and input data
\ifnotpaper (\citeyear[p.~467]{PetersAndPedrycz2000};
    \citealp[p.~121]{Patton2006}) \else \cite[p.~467]{PetersAndPedrycz2000},
    \cite[p.~121]{Patton2006} \fi on the code's behaviour. Exhaustive testing
...
% Discrep count (SYNS, WRONG): {vanVliet2000} | {IEEE2022} ISTQB {Patton2006}
...
\Cref{tab:ieeeTestTerms} is useful, it is not without its faults. One issue,
which is not inherent to the categorization itself, is the fact that it is not
used consistently (see \Cref{tab:otherTestTerms}). The most blatant example
of this is that \citet[p.~286]{IEEE2017} describe mutation testing as a
...
\Cref{tab:multiCats}; of these, experience-based testing is of particular note.
\expBasedCatMain{}
These authors previously say ``experience-based testing practices like
...
In addition to contradicting other authors (including themselves in
\citeyearpar[p.~34]{IEEE2022}!), it also implies that ``experience-based test
design techniques'' are techniques used by the \emph{practice} of
experience-based testing, not that experience-based testing is \emph{itself} a
test technique. If this is the case, it makes the distinction between
``practice'' and ``technique'' less useful, which may explain why experience-based
testing is categorized inconsistently in the literature\thesisissueref{64}.
...
\citet[p.~34\ifnotpaper, emphasis added\fi]{IEEE2022} \multAuthHelper{say}
``[another standard \citep{IEEE2021}] describes the experience-based test
\emph{design technique} of error guessing. Other experience-based test
\emph{practices} include (but are not limited to) exploratory testing \dots,
tours, attacks, and checklist-based testing''; this seems to imply that error
guessing is both a technique \emph{and} a practice, which does not make sense
if these categories are orthogonal. Similarly, the conflicting categorizations
of beta testing in \Cref{tab:multiCats} may also propagate to closed beta
testing and open beta testing; since this is an inferrence, it is omitted from
that table and instead included in \Cref{tab:infMultiCats}. These kinds of
inconsistencies between parent and child test approach categorizations may
indicate that categories are not transitive or that more thought must be given
to them.
...
\ifnotpaper
    ...
    \begin{itemize}
        ...
        \item Recovery Testing* (Failover/Recovery Testing, Back-up/Recovery
              ...
              Recovery*; see \Cref{recov-discrep})
...
% Discrep count (CATS, CONTRA): {IEEE2022} {IEEE2021} {IEEE2016} implied by {IEEE2017} {SWEBOK2024} {IEEE2022} | {IEEE2022} {SWEBOK2024} {SouzaEtAl2017} {IEEE2017}
