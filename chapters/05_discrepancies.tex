...
causing confusion. We identified \parSynAll{} of these pairs through automatic
analysis of the generated graphs. The \ifnotpaper \parSynOne{} pairs where at
      least one \else % \parSynBoth{}
      seven pairs where each \fi relation has a concrete
source are given in \Cref{tab:parSyns}.

\ifnotpaper
      \begin{landscape}
            \ifnotpaper\discrepsTable{}\fi
            \input{build/parSyns}
      \end{landscape}
      \input{build/parSynsExtra}
...
\ifnotpaper
      ...
      \begin{itemize}
            ...
            \item \citeauthor says that reviews are ``the process[es] under
                  which static white-box testing is performed''
                  \citeyearpar[p.~92]{Patton2006}, but \citeauthor{vanVliet2000}
                  also gives correctness proofs \citeyearpar[pp.~418-419].
            \item \citeauthor{PetersAndPedrycz2000} claim that ``structural
                  testing subsumes white box testing'', but they seem to
                  describe the same thing: they say ``structure tests are aimed
                  at exercising the internal logic of a software system'' and
                  ``in white box testing \dots, using detailed knowledge of
                  code, one creates a battery of tests in such a way that they
                  exercise all components of the code (say, statements, branches,
                  paths)'' on the same page \citep[p.~447]{PetersAndPedrycz2000}!
            ...
            \item Likewise, ``walkthroughs'' and ``structured walkthroughs''
                  ...
                  \citeauthor{PetersAndPedrycz2000} implies that they are
