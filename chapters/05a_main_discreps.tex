\begin{enumerate}
    \item % Discrep count (PARS, CONTRA): {IEEE2022} {Firesmith2015} | {IEEE2017} {SWEBOK2024} {BarbosaEtAl2006} | {vanVliet2000}
    ...
    \item % Discrep count (DEFS, OVER): {IEEE2022}
    ...
    \item % Discrep count (TERMS, WRONG): {IEEE2021} {SWEBOK2024} {SWEBOK2014}
    ...
    \item % Discrep count (TERMS, WRONG): {IEEE2017} {Firesmith2015} {vanVliet2000}
          % Discrep count (SYNS, WRONG): {IEEE2017} {vanVliet2000}
    ...
    \item % Discrep count (SYNS, OVER): {IEEE2017} {IEEE2013} | {IEEE2022} {IEEE2017} {Perry2006}
          ...
          but they have other definitions as well. ``Test level'' can also
    ...
    \item % Discrep count (CATS, WRONG): {IEEE2016}
          ...
          \citetext{p.~6} is incorrect.\todo{This discrepancy is based on the
              manual/automated categories; should this be moved?}
    \item % Discrep count (DEFS, OVER): {IEEE2010}
    ...
    \item % Discrep count (DEFS, OVER): {IEEE2010} {SWEBOK2024}
          ...
          % Discrep count (DEFS, AMBI): {IEEE2010} {SWEBOK2024} | {IEEE2010} ISTQB
          which seems to refer to this definition of ``error''.

          % STD | META
    \item % Discrep count (CATS, WRONG): ISTQB
          % Ground truth: {IEEE2022} {IEEE2021} {IEEE2017}
    ...
    \item % Discrep count (DEFS, CONTRA): {SWEBOK2024} {Patton2006} | {IEEE2017}
    ...
    \item % Discrep count (DEFS, CONTRA): {IEEE2017} | {SWEBOK2024} | ISTQB
          \alphaDiscrep{}
    \item % Discrep count (SYNS, CONTRA): ISTQB | {IEEE2022}
    ...
    \item % Discrep count (SYNS, CONTRA): ISTQB implied by {Gerrard2000a} | {SWEBOK2024}
    ...
    \item % Discrep count (TERMS, WRONG): {Firesmith2015}
          ...
          relevant\dots\todo{Is this a scope discrep?}
    ...
    \item % Discrep count (SYNS, WRONG): {Patton2006}
          % Ground truth: {IEEE2010} {SWEBOK2024} {vanVliet2000}
          The differences between the terms ``error'', ``failure'' and
          ``fault'' are significant and meaningful \ifnotpaper
              (\citealp[pp.~128, 139--140]{IEEE2010};
              \citealp[p.~12\=/3]{SWEBOK2024};
              \citealp[pp.~399--400]{vanVliet2000})\else
              \cite[p.~12\=/3]{SWEBOK2024}, \cite[pp.~128, 139--140]{IEEE2010},
              \cite[pp.~399--400]{vanVliet2000}\fi, but \citet[p.~14]{Patton2006}
          does away with them, saying ``there's no reason to dice words'' and
          to ``just call it what it is and get on with it''. These terms, along
          with ``defect'', ``problem'', ``incident'', ``anomaly'', ``variance'',
          ``inconsistency'', ``feature'' (!), and ``a list of unmentionable
          terms'' are abandoned in favour of the chosen term ``bug''
          \citetext{pp.~13--14}.
    ...
    \item % Discrep count (DEFS, CONTRA): {vanVliet2000} | implied by {Patton2006}
          ...
          % TEXT | PAPER
    \item % Discrep count (DEFS, WRONG): {Kam2008}
          % Ground truth: ISTQB
    ...
    \item % Discrep count (CATS, CONTRA): {Kam2008} | implied by {IEEE2021}
