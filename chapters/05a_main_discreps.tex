\begin{enumerate}
    ...
    \item % Discrep count (TERMS, WRONG): {SWEBOK2024}
          ...
          Since ``error'' and ``fault'' have distinct meanings \ifnotpaper
              (\citealp[pp.~128, 140]{IEEE2010}; \citealp[p.~12\=/3]{SWEBOK2024};
              \citealp[pp.~399--400]{vanVliet2000})\else
              \cite[pp.~128, 140]{IEEE2010}, \cite[p.~12\=/3]{SWEBOK2024},
              \cite[pp.~399--400]{vanVliet2000}\fi, error guessing should
          instead be called ``fault guessing'' if it ``anticipate[s] the most
          plausible faults in each \acs{sut}'' \citep[p.~5\=/13]{SWEBOK2024}.
