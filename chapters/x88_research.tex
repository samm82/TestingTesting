...
\section{Existing Taxonomies, Ontologies, and the State of Practice}\label{exist-tax}
...
\begin{itemize}
    ...
    \item Test Plan: a ``document describing the scope, approach, resources,
          and schedule of intended test activities'' (\citealp{ISO_IEC2014}%
          \todo{OG IEEE, 1990}; \citealp[pp.~412\==413]{vanVliet2000}) in more
          detail that the \acs{vnv} Plan (pp.~412\==413);
    ...
    \item Test Case documentation: ``specifies inputs, predicted outputs and
          ...
          \citep[p.~413]{vanVliet2000}
    \item Test Procedure: ``detailed instructions for the set-up, execution,
          and evaluation of results for a given test case''
          (\citealp{ISO_IEC2014}\todo{OG IEEE, 1990}; similar in
          \citealp[p.~413]{vanVliet2000})
...
\begin{itemize}
    \item The scope of testing is very dependent on what type of software
          ...
          is present\thesisissuenote{54}. For example, a method table
...
\begin{itemize}
    ...
    \item Undecidability \citep[p.~439]{PetersAndPedrycz2000}: it is
          ...
          \citep[p.~4]{Gurfinkel2017}), so ``automatic testing
          can't be guaranteed to always work'' for all properties
          \citep{Nelson1999} \todo{Add paragraph/section number?}
\end{itemize}
