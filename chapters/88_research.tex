...
\textbf{(Multiple) condition coverage} does not subsume and is not subsumed by
path coverage \citep[p.~433]{vanVliet2000} and ``may be quite challenging''
since ``if each subcondition is viewed as a single input, then this
\dots\ is analogous to exhaustive testing''; however, there is usually a
manageable number of subconditions \citep[p.~464]{PetersAndPedrycz2000}.
