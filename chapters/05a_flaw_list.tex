\begin{enumerate}
    ...
    \item % Flaw count (WRONG, LABELS): {IEEE2021c} | {IEEE2022} {IEEE2021c} {IEEE2013} {SWEBOK2025} {Firesmith2015} {Patton2006}
          ...
          (see \Cref{error-fault-failure}),
          %   \ifnotpaper (\citealp[pp.~124, 165, 178\==179]{IEEE2017};
          ...
          ``error guessing''---a term used by multiple sources \citep{IEEE2022,%
              IEEE2021c,IEEE2013,SWEBOK2025,Firesmith2015,Patton2006}---should
          either be called:
          \begin{itemize}
    ...
    \item % Flaw count (WRONG, LABELS): {IEEE2017} {IEEE2010} | {IEEE2017} {Firesmith2015} {vanVliet2000}
          % Flaw count (WRONG, SYNS): {IEEE2017} {IEEE2010} | {IEEE2017} {vanVliet2000}
    ...
    \item % Flaw count (OVER, LABELS): {IEEE2017} {IEEE2010} {SWEBOK2025}
          ...
          % Flaw count (AMBI, DEFS): {IEEE2010} {SWEBOK2025} | {IEEE2010} ISTQB
          ...
          % Flaw count (CONTRA, SYNS): {IEEE2017} | {IEEE2017}
    ...
    \item % Flaw count (CONTRA, SYNS): {SWEBOK2025} | {Firesmith2015}
          Similarly, ``pair testing'' and ``buddy testing'' are given as
          synonyms by \citet[p.~5\=/14]{SWEBOK2025} but listed
          separately by \citet[pp.~36, 39]{Firesmith2015}.
    \item % Flaw count (AMBI, DEFS): {IEEE2017} {IEEE2010} {SWEBOK2025} | {SWEBOK2025}
          ...
          incorrect result'' (\citealp[p.~165]{IEEE2017}\todo{OG 2009};
    ...
    \item % Flaw count (AMBI, LABELS): {IEEE2024} {IEEE2010} | {SWEBOK2025}
          % Assertion: {vanVliet2000}
          Similarly, \citet[p.~4\=/11]{SWEBOK2025} says ``\emph{fault tolerance}
          ...
          ``fault'' have distinct meanings (\citealp[p.~36]{IEEE2024};
          %   \citealp[pp.~165\todo{OG 2009}, 179]{IEEE2017};
          \citealp[pp.~128, 140]{IEEE2010}; % \citealp[p.~12\=/3]{SWEBOK2025}%
          %   \footnote{\citet[p.~12\=/3]{SWEBOK2025} references the definitions
          %       given in \citep[pp.~165, 179]{IEEE2017}; while we would usually
          %       omit the former in favour of the original source, we include it
          %       here as an example of a flaw within a document.};
    ...
    \item % Flaw count (WRONG, TRACE): ISTQB
          ...
          type'' does not provide a definition.
    \item % Flaw count (WRONG, TRACE): ISTQB
    ...
    \item % Flaw count (WRONG, SYNS): {IEEE2017} {IEEE2010} {vanVliet2000} | {Patton2006}
          % Label bug-patton
          The differences between the terms ``error'', ``failure'',
          ``fault'', and ``defect'' are significant and meaningful
          (\citealp[pp.~124, 165, 178\==179]{IEEE2017};
          \citeyear[pp.~96, 128, 139\==140]{IEEE2010};
    ...
    \item % Flaw count (WRONG, PARS): {IEEE2021c} {IEEE2017} {vanVliet2000} | implied by {PetersAndPedrycz2000}
          ...
          coverage \emph{and} \acs{p-use} coverage; these terms are
          defined as follows:
          \begin{itemize}
              \item \acf{p-use}: a ``data use associated with the decision
                    outcome of the predicate portion of a decision statement''
                    (\citealp[p.~5]{IEEE2021c}; \citeyear[p.~332]{IEEE2017};
                    similar in \citeyear[p.~27]{IEEE2021c})%; e.g., the use of a
                    % variable in a conditional \citep[p.~424]{vanVliet2000}
                    % like ``a while-loop or if-then-else'' \citep[p.~27]{IEEE2021c}
                    .
              \item \acf{c-use}: a ``use of the value of a variable in any
                    type of statement'' (\citeyear[p.~2]{IEEE2021c};
                    \citeyear[p.~83]{IEEE2017}), ``as an input to the
                    computation of the definition of any variable or of an
                    output'' (\citeyear[p.~27]{IEEE2021c}; similar in
                    \citealp[p.~424]{vanVliet2000}), or not as a \acs{p-use}
                    \citep[p.~424; implied by][p.~27]{IEEE2021c}%; e.g., the
                    % use of a variable in a computation or I/O statement
                    % \citep[p.~424]{vanVliet2000}
                    .
          \end{itemize}
          Therefore, \citepos[Fig.~12.31]{PetersAndPedrycz2000} claim seems
          incorrect, since decisions are the result of \acsp{p-use} and
