\begin{enumerate}
    ...
    \item % Flaw count (CONTRA, CATS): {IEEE2021b} | {IEEE2022}
          % Label static-level-flaw
          \citeauthor{IEEE2021b} \staticLevelBlurb{} but make it its own test
          approach category in \citeyearpar[p.~10, 23, Fig.~2]{IEEE2022}.
    ...
    \item % Flaw count (CONTRA, SYNS): {IEEE2021c} | implied by {IEEE2021c}
          ...
          (Fig.~G.1, Secs.~5.3.2, 5.3.3, Annex~C.2.2). However, (Fig.~F.1)
          implies that these terms are synonyms, contradicting this separation
          and making decision testing's relations ambiguous.
    ...
    \item % Flaw count (WRONG, CATS): {IEEE2016} | {IEEE2022} {IEEE2016}
          ...
          The former statement could also be interpreted as implying that
    ...
    \item % Flaw count (AMBI, PARS): {PMBOK2013} | {PMBOK2013}
          ...
          introduces ambiguity at best and creates a directed cyclic
          parent-child relation (which violates our definition in
          \Cref{par-chd-rels}) at worst.
    \item % Flaw count (MISS, LABELS): {PMBOK2013}
          ...
          % META | TEXT
    \item % Flaw count (WRONG, TRACE): {Black2009} | {Black2009}
          The publisher of \citet{Black2009} is spelled wrong (``Rock Nook Inc.''\
          instead of ``Rocky Nook Inc.'')\ on page six but spelled correctly elsewhere.
    ...
    \item % Flaw count (MISS, DEFS): {IEEE2021c} {IEEE2017} {PetersAndPedrycz2000} implied by {vanVliet2000} | {vanVliet2000}
          \Citet[pp.~425, 432\==433]{vanVliet2000} defines many kinds of data
          flow coverage and describes their relations, including all-\acsp{p-use},
          all-\acsp{p-use}/some-\acsp{c-use}, and all-\acsp{c-use}/some-\acsp{p-use},
          but excludes all-\acsp{c-use}, which is implied by these definitions and
    ...
    \item % Flaw count (CONTRA, DEFS): {IEEE2017} | {SWEBOK2014} | {vanVliet2000}
          ...
          % Flaw count (AMBI, DEFS): {IEEE2017} | {SWEBOK2014} | {vanVliet2000}
          some uncertainty (``usually'', ``may or may not'', and ``often'',
          respectively).
    ...
    \item % Flaw count (WRONG, SCOPE): {Gerrard2000b}
    ...
    \item % Flaw count (WRONG, SYNS): {IEEE2017} {Hetzel1988} {SakamotoEtAl2013} | {SneedAndGöschl2000}
          % Label dubious-syns
          \citet[p.~3]{SneedAndGöschl2000} give
          ...
          ``black-box integration testing''. They likely take the ``implicit''
          claims from the original source they cite \citep{Hetzel1988} too far;
          for example, ``as a practical matter, \emph{most} system testing
          relies on the black-box perspective'' \citetext{p.~11, emphasis added}.
          However, this does not mean that system testing is \emph{always} the
          same as black-box testing.
    \item % Flaw count (WRONG, SYNS): {SneedAndGöschl2000}
          % Label dubious-red-box-syn
          \citepos[p.~3]{SneedAndGöschl2000} incorrect claims about test level
          synonyms from \flawref{dubious-syns} make their claim that
          ``red-box testing'' is a synonym for ``acceptance testing'' lose
          credibility.
    \item % Flaw count (WRONG, TRACE): {SneedAndGöschl2000}
          \citet[p.~3]{SneedAndGöschl2000} assert that ``module testing'' can
          also be called ``class testing'', but this claim does not appear in
          the source they cite \citep{Hetzel1988}.
          % Flaw count (WRONG, SYNS): implied by {Hetzel1988} {Kam2008} | implied by {SneedAndGöschl2000}
          They might have meant to say that ``module testing'' and ``program
          testing'' are synonyms, which is implied by this source (p.~73) and
          supported by \citet[p.~46]{Kam2008}.
\end{enumerate}