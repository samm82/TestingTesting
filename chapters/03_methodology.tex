...
\ifnotpaper
    ...
    In \Cref{auto-flaw-analysis}, we describe some subsets of flaws that we
    ...
    with comments we can parse automatically (as described in
    \Cref{aug-flaw-analysis}) %, allowing us to analyze them more broadly. These comments have
    ...
    \texttt{(WRONG, TRACE)} and one for \texttt{(MISS, DEFS)}\thesisissueref{157}.
...
we achieved something close to a complete taxonomy! However, this was
infeasible due to time constraints. We also considered property-based testing
...
were surprised that it is not mentioned by any sources we investigated. With
more time, we would find definitions for all uncovered terms as described in
\Cref{undef-terms}, \ifnotpaper as well as those that did not arise as described
    in \Cref{future-miss-terms}, \fi but unfortunately, we impose our stopping
point artificially.
