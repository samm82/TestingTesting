...
\input{build/methodOverviewList}
...
\citep{IEEE2017} and three versions of the \acf{swebok} \ifnotpaper
    (\citealp{SWEBOK2014,SWEBOK2024,SWEBOK2025}; see \Cref{swebok-update})\else
    \cite{SWEBOK2014,SWEBOK2024,SWEBOK2025} (see \Cref{swebok-update})\fi.
...
categories are orthogonal (i.e., that A/B testing \emph{cannot} both be a test
practice \emph{and} a test type), so we consider this to be an inferred flaw that
we automatically detect and document \ifnotpaper (see \Cref{auto-flaw-detect,%
...
significant nuance that we need to document. We find the following
non-mutually exclusive cases of implicit information from the literature:

\begin{enumerate}
    \item \textbf{The information follows logically}
          % from the source and information from others,
    ...
    \item \textbf{The information is dubious}
          %; while it is present in the literature, there is
          with reason to doubt its accuracy.
\end{enumerate}
...
%           \refHelper \citet[emphasis added]{ISO_IEC2014c}
%           \multiAuthHelper{define} ``regression
...
%           %   flaw.
% \end{description}
These cases are often, but not always, indicated in the literature by (at
least) one of the following keywords: \impKeywords{}. Even when they are
not, we use them in \ourApproachGlossary{} \ifnotpaper
(see \Cref{tab:impCaseKeywords}) \fi to more completely summarize the
software testing literature without getting distracted by less
relevant details. This also helps us identify implicit
information during later analysis. While ``implicit'' only
describes the first of these cases, we use it (as well as ``implied by'' when
describing sources of information) as a shorthand for all ``not explicit''
information throughout this \docType{} for clarity.

\ifnotpaper
    Some test approaches may or may not include the word ``testing'' in their
    names or may not be test approaches at all! For example, the term
    ...
    suffix ``~(Testing)'' (space included; see \Cref{tab:impCaseKeywords})
    when it makes sense to do so and consider the test approach to be implied.
    ...
    but is also an example of incorrect traceability information.
    We record both of these views\thesisissuenote{157} but
    ...
    display this flaw more meaningfully as an example of incorrect traceability
    information (\flawref{see-ref-missing}).
