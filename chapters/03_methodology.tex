...
} \citep{SWEBOK2024}\ifblind{}{---as \suggSrcs{}\todo{
...
documents (\Cref{papers}). A summary of how many sources comprise each tier is
given in \Cref{fig:sourceSummary}. We often use papers to ``fill in'' missing
...
flaws within these standards (see \Cref{tab:sntxDiscreps,tab:smntcDiscreps})!
...
techniques, practices, or static testing \citeyearpar[Fig.~2; see
    \Cref{tab:ieeeCats}]{IEEE2022}. However, we consider static testing to be a
...
level of analysis\ifnotpaper\ (see \Cref{static-test})\fi, but may fold them
into other categories as appropriate, although some teams may wish to keep them
separate based on their specific context.

Of the five primary categories we use (see \Cref{tab:ieeeCats}), ``level''
...
system testing, and acceptance testing as examples of test levels \ifnotpaper
    ...
    \cite[p.~218]{KuļešovsEtAl2013}\todo{OG Black, 2009}\fi,
...
\ifnotpaper For this reason, these ``categorizations'' are marked with a
    ...
    categories are not used consistently (see \discrepref{manual-or-keyword,%
...
\ifnotpaper
    ...
    for completeness; inferred flaws are given in \Cref{infer-discreps}
