...
an inferred flaw \ifnotpaper (see \Cref{infers,auto-flaw-analysis}) and appear
    in \Cref{tab:infMultiCats}\else (details omitted for brevity)\fi.
...
type (e.g., reusability testing). \ifnotpaper
    ...
    \recFigs{} (as described in \Cref{graph-gen}).\fi
...
\thesisissueref{39})\ \fi or quite broad\ifnotpaper\ (such as attacks; see
...
(see \Cref{infers}). \ifnotpaper Even assuming this is true, some kinds of
    requirements do not apply to the code itself, and as such are out of
    scope\thesisissueref{43}, such as:
    \begin{itemize}
