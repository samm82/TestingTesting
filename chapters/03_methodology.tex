...
time'' but not always. As an example, \Cref{tab:parSyns} contains relations
...
We track terminology used in the literature by building glossaries.
Each test approach gets its own row in our test approach glossary where we
...
We use similar methodologies to track software qualities \ifnotpaper (see
    \Cref{qual-test}) \fi and supplementary terminology (either shared by
multiple approaches or too complicated to explain inline)
