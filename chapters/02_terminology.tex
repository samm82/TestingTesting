...
\begin{enumerate}
    ...
    \item \phantomsection{}\label{decide-app} decide on an approach,
    \item develop the tests,
    \item determine the expected results,
    \item \phantomsection{}\label{run-tests} run the tests, and
    \item \phantomsection{}\label{cmp-results} compare the expected results to
...
the results of step~\ref{cmp-results} \ifnotpaper (\citealp[p.~10]{IEEE2022};
...
Of course, the approach chosen in step~\ref{decide-app} influences what
...
values'' \citeyearpar[p.~465]{IEEE2017} used in step~\ref{run-tests}. The only
...
manageable subsets, we use these categories.
...
\begin{enumerate}
    ...
    \item We introduce a non-orthogonal category of ``artifact''\thesisissueref{44,119,39}
          ...
          another category as flaws\thesisissueref{119} (see \Cref{multiCats}).
\end{enumerate}

\ifnotpaper\label{orth-approach}
Based on their definitions and usage, these categories seem to be orthogonal.
For example, ``a test type can be
...
practice \citep[pp.~20, 22\ifnotpaper; implied by p.~33\fi]{IEEE2022}; see
\Cref{app-orth-test} for more detailed discussion.
\fi
...
their multi-faceted ``parent(s)''. This nomenclature also extends to other
categories (such as ``subtype''; see \Cref{cats-def,tab:ieeeCats}) and software
qualities (``subquality''\ifnotpaper; see \Cref{qual-test}\fi).
...
partial order, and a given ``child'' approach $c$ may have more than one
...
is violated as a ``flaw''\footnote{A small literature review reveals that
    established standards (see \Cref{stds}) only use the term ``flaw'' to refer
    to requirements \citep[p.~38]{IEEE2022}, design \citetext{p.~43}, ``system
    ...
    engineering's testing literature.}
%     , as it implies that something is wrong with the literature and an
% opportunity to improve it, 
while avoiding words that are ``overloaded with too many meanings''
like ``error'' and ``fault'' \citep[p.~12\=/3; see
    \Cref{error-fault-failure}]{SWEBOK2024}.

Referring to a flaw with more than one source (such as a contradiction or an
overlap) as a ``flaw between two sources'' is awkward\thesisissueref{140}, so
we refer to this more intuitively as an ``inconsistency'' between the sources.
This clearly indicates that there is disagreement between
...
correct\thesisissueref{140,151}! Note that these cases are not categorized as
``mistakes'' if finding this ground truth requires analysis that has not been
performed yet.
...
present our observations according to these two views in \Cref{tab:flawMnfsts,%
    tab:flawDmns}. We explicitly define some of these domains in previous
...
\phantomsection{}\label{scope-flaw-def}%
Additionally, some information is presented with an incorrect scope and
sometimes should not have been included at all (see \flawref{assert-truth})!
...
\begin{description}
    \item[1. The information follows logically.]
          ...
          \Cref{cats-def}). We therefore consider these categorizations to be
          implicit, as in \Cref{tab:multiCats\ifnotpaper,tab:infMultiCats\fi}.
...
\ifnotpaper Additionally, \Cref{tab:parSyns,tab:multiCats,tab:infMultiCats}
    ...
    are marked by the phrases ``implied by''.
    ...
    explicitly or otherwise, by a source. For example, \citeauthor{Gerrard2000a}
    describes large scale integration testing and legacy system integration
    testing in \citeyearpar[p.~30]{Gerrard2000b} and
    ...
    (see \Cref{cats-def}). Although these data originate from our judgement,
    we document them for completeness, using the phrase ``inferred from'' when
    relevant. Additionally, all data in our glossaries without a citation are
    inferred, such as algebraic testing \citep[Fig.~12.2]{PetersAndPedrycz2000}
    being a child of mathematical-based testing. We present inferred flaws in
    \Cref{infer-flaws} and colour inferred approaches and relations grey in our
    generated graphs, such as \recFigs{}.
\fi
