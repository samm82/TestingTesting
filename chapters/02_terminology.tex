...
for ``test level'' \ifnotpaper
    (see \flawref{level-phase-syns,stage-level-syns}).\label{orth-approach}
\else (see relevant synonym flaws in \Cref{syns}). \fi
...
test approaches from different categories\ifnotpaper
    ...
    ; see \Cref{app-orth-test} for more detailed discussion\fi.
...
\emph{and} another category as flaws\thesisissueref{119} (see \Cref{multiCats}).
