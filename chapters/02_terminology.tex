...
(\Cref{mnfst-def}) and domains (\Cref{dmn-def}) and use %\thesisissuetodo{155}
...
% \thesisissuetodo{176}
, of information (\Cref{explicitness}) found in sources and
...
Software testing is an ``activity in which a system or component is executed
...
\testingDefRefs{}\ifnotpaper, usually ``with the intent of finding errors''
...
% \citeyear[p.~6]{IEEE2021c}; \citeyear[p.~473]{IEEE2017})\else
% \cite[p.~473]{IEEE2017}, \cite[p.~10]{IEEE2022}, \cite[p.~6]{IEEE2021c}\fi.
When this process reveals errors, ``the faults causing them are what can and
...
most non-trivial situations \ifnotpaper (\citeyear[p.~4]{IEEE2022};
    \citealp[p.~5\=/5]{SWEBOK2025}; \citealp[pp.~439, 461]{PetersAndPedrycz2000};
    \citealp[p.~421]{vanVliet2000})\else
    \cite[pp.~439, 461]{PetersAndPedrycz2000}, \cite[p.~421]{vanVliet2000},
    \cite[p.~5\=/5]{SWEBOK2025}, \cite[p.~4]{IEEE2022}\fi, so multiple
...
system testing, and acceptance testing as examples of test levels \ifnotpaper
    ...
    \citealp[pp.~12, 14, 18, 24, 62, 178]{Black2009};
    \citealp[pp.~807\==808]{Perry2006}; \citealp[pp.~443\==445]{PetersAndPedrycz2000};
    \citealp[pp.~9, 13]{Gerrard2000a})\else \cite[pp.~443\==445]{PetersAndPedrycz2000},
    \cite[pp.~5\=/6 to 5\=/7]{SWEBOK2025}, \cite{ISTQB},
    \cite[pp.~807\==808]{Perry2006}, \cite[pp.~9, 13]{Gerrard2000a},
    \cite[pp.~12, 14, 18, 24, 62, 178]{Black2009}\fi.%
\phantomsection{}\label{orth-approach}
...
as follows: %\thesisissuetodo{21}
\begin{itemize}
    \item \textbf{Level:} When in the \ifnotpaper software \fi life cycle the
          test is performed
...
category, we omit it as a category since it seems non-orthogonal to the others
and thus less helpful for grouping test approaches. These authors even
\staticLevelBlurb{; see \flawref{static-level-flaw}}! \ifnotpaper Other
    schemas (see \Cref{alt-cats}) consider static testing to be an orthogonal
    category or out of scope entirely (see \Cref{static-test}). \fi We also
introduce a ``supplemental'' category of
``artifact''\thesisissuenote{44,119,39} since some terms can refer both to the
application of a test approach \emph{and} to the resulting document(s).
\ifnotpaper
    Some sources explicitly distinguish between the two, such as between
    ``conformity evaluation'' and ``conformity evaluation report''
    \citeyearpar[p.~93]{IEEE2017}, but most do not.
...
\emph{and} another category as flaws in \Cref{multiCats}. % \thesisissuetodo{119}
...
% \ifnotpaper
%     Additionally, we identify ``test metrics''\thesisissuetodo{21,22} that
...
\ifnotpaper
    ...
    \citeyear[p.~431]{IEEE2017}; \citealp[p.~5\=/10]{SWEBOK2025};
    \citealpISTQB{}; \citealp[pp.~46\==47\noHyphenFn]{Firesmith2015};
    \citealp[p.~399]{vanVliet2000}; \citealp[p.~344]{SakamotoEtAl2013})\else
    \cite[pp.~46\==47\noHyphenFn]{Firesmith2015}, \cite[p.~399]{vanVliet2000},
    \cite[p.~5\=/10]{SWEBOK2025}, \cite[p.~431]{IEEE2017},
...
%           \fi
%     \item ``domain testing'' \citep[p.~5\=/10]{SWEBOK2025}
...
% Similarly, \citet[p.~5\=/10]{SWEBOK2025} says ``\emph{specification-based
...
%         \citep[Fig.~2]{IEEE2021c} and \citep[pp.~5\=/11]{SWEBOK2025}%
%     \else
%         \cite[pp.~5\=/11]{SWEBOK2025} and \cite[Fig.~2]{IEEE2021c}%
...
% this \citep[pp.~5\=/10 to 5\=/12]{SWEBOK2025}!
...
cycles, although undirected cycles may exist (for example, if a child and its
parent share the same parent).
...
wrong; see \Cref{dmn-def}). These are orthogonal % \thesisissuetodo{155}
...
flaws: \emph{how} information is ``wrong''. We observe the % \thesisissuetodo{155}
``manifestations'' described in \Cref{tab:flawMnfstDefs} throughout the
literature, and give each a unique key for later analysis and discussion.
\ifnotpaper%
    We also use this manifestation view when referencing a specific flaw%
    % from our full lists of flaws in \Cref{flaws-full}
    ; for instance, \flawref{alpha-def} presents contradictory
    definitions of ``alpha testing''.
    % We list our manifestations in descending order of severity, although this is
    % partially subjective. While some may disagree with our ranking, it is clear that
    ...
    % remaining manifestation types for the flaw.
\fi
Note that some flaws involve
...
requires analysis that we have not yet performed.
...
as a ``flaw between two sources'' is awkward. We instead % \thesisissuetodo{140}
...
correct! % \thesisissuetodo{140,151}

\phantomsection{}\label{one-src-flaws}
Other flaws only involve one source, but \oneSrcDistinct{}\thesisissuenote{137,138}.
...
these flaws are \emph{actually} problematic% \thesisissuetodo{176}
...
observing how terms are used in practice: % \thesisissuetodo{89}
