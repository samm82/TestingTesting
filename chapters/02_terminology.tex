...
\dots{} and for various types of testing'' \citeyearpar[p.~4]{IEEE2016}. We
assume this to be true when identifying flaws and may assess the orthogonality
of these categories more rigorously in the future. For now, this assumption
...
``static testing''. However, this seems to be non-orthogonal to the others and
...
in this way, there are some outliers. For example, we categorize some test
approaches as ``artifacts'' in addition to \citet{IEEE2022}'s categories%
\thesisissueref{44,119,39}, since some terms can refer to the application of a
test approach \emph{as well as} the resulting document(s). Because of this, we
...
\ifnotpaper\afterpage{\begin{landscape}%
    ...
    \Cref{tab:otherCategorizations} for completeness. The source for each basis
    also lists the example approaches given; in some cases, it provides
    additional approaches that we omit for brevity. This source categorizes
    all these example approaches as the given ``Parent IEEE Category'' unless
    stated otherwise. For example, in the first row, \citet[pp.~22, 35]{IEEE2022}
