...
\multiAuthHelper{include} ``static testing''. However, this seems to be
...
an ``artifact'' category is an additional option\thesisissueref{44,119,39},
since some terms can refer to the application of a test approach
\emph{as well as} the resulting document(s). Because of this, we do \emph{not}
include test approachs categorized as a category from \Cref{tab:ieeeCats}
\emph{and} an artifact as flaws\thesisissueref{119} in \Cref{multiCats}.
\ifnotpaper
    Additionally, we identified\ptq{} ``test metrics''\thesisissueref{21,22},
    ...
    We instead capture the related test approaches that seek to maximize these
...
\ifnotpaper\afterpage{\begin{landscape}%
    ...
    the notion of test approaches families in more detail in
