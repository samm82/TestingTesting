...
Of course, the approach the tester chooses influences how the tests are
developed, so it is important that they are unambiguous, consistent, and
defined correctly. This is where we focus our efforts: tracking flaws with
how the literature describes approaches to testing, or ``test approaches''.
These are ``high-level test implementation choice[s]'' \citep[p.~10]{IEEE2022}
that are used to ``pick the particular test case values''
\citeyearpar[p.~465]{IEEE2017}. Since the only approach that can ``fully'' test
a system is exhaustive testing which is infeasible in most non-trivial
...
multiple approaches \ifnotpaper \citealp[p.~18]{IEEE2022} \else
...
\subsection{Synonym Relations}\label{syn-rels}
...
\subsection{Parent-Child Relations}\label{par-chd-rels}
...
partial order, and a given $c$ may have more than one parent $p$.
