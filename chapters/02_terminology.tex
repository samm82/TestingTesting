...
(\Cref{flaw-def}). To further prevent bias, we do not invent or add our own
...
(\Cref{par-chd-rels}) and while these terms are not used by the literature, we
observe flaws having both manifestations (\Cref{mnfst-def}) and domains
(\Cref{dmn-def}). \conceptResults{}
