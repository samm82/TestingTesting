...
what we mean by the term.\latertodo{Once a definition of ``flaw'' congeals, lead with it!}
...
\footnote{A small literature review reveals that established standards
    (see \Cref{stds}) only use ``flaw'' to refer to requirements
    \citep[p.~38]{IEEE2022}, design \citetext{p.~43}, both of these along with
    ``system security procedures \dots{} and internal controls'' under the term
    ``vulnerability'' \citep[p.~194]{IEEE2012}, or code itself \citetext{p.~92}.},
especially since we use it to describe software's supporting documentation, not
software itself. \refHelper \citet[p.~7\=/9]{SWEBOK2024} mentions that
``techniques and indicators can help engineers measure technical debt,
including \dots{} the number of engineering flaws and violations'', which
aligns with our critique of software engineering's testing literature.
Therefore, we use the term
``flaw'' to describe anything ``wrong'' with software testing literature.
...
\input{assets/tables/sntxFlawKinds}

\latertodo{Update references to flaw sections once their format is locked down;
    see \thesisissuerefhelper{134}.}
...
\input{assets/tables/smntcFlawKinds}
...
\Cref{tab:smntcFlawKinds}, and tracking these uncovers which knowledge domains
...
present our observations according to these two views in \Cref{tab:sntxFlaws,%
    tab:smntcFlaws}. We explicitly define some of these domains in future
...
nuance.\latertodo{Explain later once we (a) move all defs here from the flaws
    section and (b) agree on what to do about the ``citation'' domain.}
