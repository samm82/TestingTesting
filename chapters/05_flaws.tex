...
context may rectify them. \ifnotpaper Additionally,
    \Cref{tab:parSyns,tab:multiCats,tab:infMultiCats} contain relations that
...
We summarize the flaws that we discover manually in \Cref{flawMnfsts} based on
their manifestation (defined in \Cref{mnfst-def}\ifnotpaper; see
\Cref{aug-flaw-analysis}\fi). This lets us keep flaws that we automatically
uncover \ifnotpaper (see \Cref{auto-flaw-analysis}) \fi separate; we summarize
these based on their domain (defined in \Cref{dmn-def}) in \Cref{flawDmns}.
...
include functional testing (\Cref{func-test-flaw}), \ifnotpaper operational
    (acceptance) testing (\Cref{oat-flaw}), \fi recovery testing
(\Cref{recov-flaw}), scalability testing (\Cref{scal-flaw}), and compatibility
testing (\Cref{compat-flaw}). \ifnotpaper Finally, we present inferred flaws,
    as described in \Cref{infers}, for completeness in \Cref{infer-flaws};
...
We find seven cases where a definition \ifnotpaper (see \Cref{defs}) \fi is
...
information on specific areas of these domains that require more investigation
in the future. The domains we focus on here are test approach categories
(\Cref{cats}), synonym relations (\Cref{syns}), \ifnotpaper \else and \fi
parent-child relations (\Cref{pars})\ifnotpaper, and definitions
(\Cref{defs})\fi.
...
\citep[p.~8]{IEEE2021b}. For example, ``the test coverage items derived by
...
\Cref{orth-approach}). These flaws mostly involve the category of ``test
...
more general sense where we would use the term ``test approach''. We list
\ifnotpaper all \else some \fi of these flaws along with their sources in
\Cref{tab:multiCats}\ifnotpaper\ for completeness\fi, but the multiple
categorizations of experience-based testing are of particular note.
...
although they ``can use \dots\ test techniques'' \citeyearpar[p.~viii]{IEEE2021b},
...
\begin{itemize}
    \item Combinatorial testing (\citealp[p.~3]{IEEE2022};
          \citeyear[p.~2]{IEEE2021b}; \citealp[p.~5-11]{SWEBOK2024})
    \item Data flow testing (\citeyear[p.~3]{IEEE2021b};
          implied by \citealp[p.~5-13]{SWEBOK2024})
    \item Performance(-related) testing (\citealp[p.~38]{IEEE2021b};
          \perfAsFamily*{})
    \item Security testing \citep[implied by][p.~40]{IEEE2021b}
...
\citeyear[pp.~7, 40, Tab.~A.1]{IEEE2021b}; \citeyear[p.~405]{IEEE2017}%
\todo{OG 2013}; implied by its quality (\citealp{ISO_IEC2023a};
\citealp[p.~13-4]{SWEBOK2024}); \citealp[p.~53]{Firesmith2015}) despite
consisting of ``a number of techniques'' \cite[p.~40]{IEEE2021b}, although
...
\citep[p.~8, Fig.~2]{IEEE2021b}.

As described in \Cref{infers}, we infer that child approaches inherit their
parents' categories. However, subapproaches of experience-based tesing, such as
error guessing and exploratory testing, are categorized ambiguously, casting
doubt on the validity of this assumption.
\refHelper \citet[p.~34\ifnotpaper, emphasis added\fi]{IEEE2022}
\multiAuthHelper{say} that a previous standard \citeyearpar{IEEE2021b}
...
orthogonal. Similarly, the conflicting categorizations of beta
testing in \Cref{tab:multiCats} may propagate to its children closed beta
testing and open beta testing. These kinds of inconsistencies between parent
and child test approach categorizations may indicate that categories are not
transitive or that more thought must be given to them. When we infer these
flaws, we exclude them from \Cref{tab:multiCatTable,tab:multiCats}, instead
including them in \Cref{tab:infMultiCats} for completeness.
\fi
...
flaws. For example, some self-referential definitions imply that a test
...
\ifnotpaper
    ...
    \citet{IEEE2022, IEEE2021b}, from the
...
\ifnotpaper
    \citetext{\citealp[pp.~15,~20,~22]{IEEE2022};
        \citeyear[pp.~7,~38,~Tab.~A.1]{IEEE2021b}; \citeyear[p.~4]{IEEE2016}}%
\else
    \cite[pp.~15,~20,~22]{IEEE2022}, \cite[pp.~7,~38,~Tab.~A.1]{IEEE2021b},
...
% Flaw count (CONTRA, CATS): {IEEE2022} {IEEE2021b} {IEEE2016} implied by {IEEE2017} {SWEBOK2024} {IEEE2022} | {IEEE2022} {SWEBOK2024} {SouzaEtAl2017} {IEEE2017}
...
\NewDocumentCommand\subDRT{s}{given as a subtype of ``disaster/recovery
    ...
    \IfBooleanTF{#1}{\citetext{p.~37}}{\citeyearpar[p.~37]{IEEE2021b}},
...
\begin{itemize}
    ...
    \item \emph{Disaster/recovery testing} evaluates if a system
          ...
          resolved'' \citeyearpar[p.~37]{IEEE2021b}.
          \begin{itemize}
              \item \texttt{(OVER, DEFS)}
                    % Flaw count (OVER, DEFS): {IEEE2017} | {IEEE2021b}
    ...
    \item \emph{Backup/recovery testing} determines the ability of a system
          ...
          \citep[p.~37]{IEEE2021b}.
          \begin{itemize}
              %   \item \texttt{(OVER, DEFS)}
              %         % Flaw count (OVER, DEFS): {IEEE2021b} | {IEEE2017}
              ...
              \item \texttt{(CONTRA, PARS)}
                    % Flaw count (CONTRA, PARS): {IEEE2021b} | {IEEE2021b}
                    This \subDRT{}
              \item \texttt{(OVER, LABELS)}
                    % Flaw count (OVER, LABELS): {IEEE2021b} | {IEEE2013}
    ...
    \item \emph{Failover/recovery testing} determines the
          ...
          \begin{itemize}
              ...
              \item \texttt{(AMBI, PARS)}
                    % Flaw count (AMBI, PARS): {IEEE2021b}
              ...
              \item \texttt{(AMBI, SYNS)}
                    % Flaw count (AMBI, SYNS): {IEEE2021b} | implied by {Firesmith2015}
...
% Flaw count (CONTRA, SYNS): {IEEE2021b} | {Firesmith2015} {Bas2024}
\citeauthor{IEEE2021b} \citeyearpar[p.~39]{IEEE2021b} give
...
% Flaw count (CONTRA, DEFS): {IEEE2021b} | implied by {ISO_IEC2023a}
\citeauthor{IEEE2021b} \citeyearpar[p.~39]{IEEE2021b} also include the external
...
% Flaw count (WRONG, SYNS): implied by {IEEE2021b} | {IEEE2022}
The ``interoperability'' element of ``compatibility testing'' is explicitly
excluded by \citet[p.~37]{IEEE2021b}, (incorrectly) implying that ``compatibility
...
% Assertion: {ISO_IEC2023a} {IEEE2022} {IEEE2021b} {IEEE2017} ISTQB
...
from one hardware \dots{} environment to another'' \citep[p.~39]{IEEE2021b}.
...
\ifnotpaper
    ...
    implied by \citet[Fig.~F.1]{IEEE2021b}\todo{OG Reid, 1996} and
