...
\Cref{fig:flawBars} shows where these flaws appear within each source tier (see
\Cref{source-tiers}) which reveals a lot about software testing literature:
\begin{enumerate}
...
\ifnotpaper
    ...
    (Note that these values align with the totals in \Cref{tab:flawMnfsts,%
        tab:flawDmns}). Each tier is responsible for a comparable number of
    flaws, although most flaws come from terminology collections. This is made
    even more noticeable when normalizing these totals by the number of
    documents in each source tier which we present in \Cref{fig:normFlawBarsSummary}.
    The relatively low numbers of flaws per document in standards and papers
    are likely due to the amount of content in these documents that we did
    \emph{not} investigate, either for scope reasons or time reasons.
...
We summarize the flaws that we discover manually \ifnotpaper (see
    \Cref{flaw-comment-analysis}) \fi in \Cref{flawMnfsts} based on their
manifestation (defined in \Cref{mnfst-def}). This lets us keep flaws that we
automatically uncover \ifnotpaper (see \Cref{auto-flaw-detect}) \fi separate;
...
alternate stopping point for our research\thesisissueref{57,81,88,125}%
\todo{These issue refs might be useful in our final documents.} since we were
...
in the literature!\ifnotpaper\ With more time, we would uncover this along with
    ...
    \Cref{future-miss-terms}), but unfortunately, we impose our stopping point
    artificially.\fi\qtodo{Present tense in this paragraph?}
