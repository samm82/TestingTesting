...
manifestations and their domains (defined in \Cref{flaw-def}).
...
\Cref{trust}). The numbers of (Exp)licit and (Imp)licit
...
\fi \flawref{tour-def-contra,tour-def}.
...
(\Cref{func-test-flaw}), \ifnotpaper \acf{operat} (\Cref{oat-flaw}), \fi
recovery testing (\Cref{recov-flaw}), scalability testing (\Cref{scal-flaw}),
and compatibility testing (\Cref{compat-flaw}). \ifnotpaper Finally, we infer
    the more subjective flaws presented in \Cref{infer-flaws} which do not
...
as presented in \Cref{tab:flawMnfstDefs}. These include mistakes (\Cref{wrong}),
...
is flawed as presented in \Cref{tab:flawDmnDefs}. This includes test approach
...
\ifnotpaper
    ...
    % Flaw count (CONTRA, SYNS): {LambdaTest2024} {BocchinoAndHamilton1996} | {Firesmith2015}
