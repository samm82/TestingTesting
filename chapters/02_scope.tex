...
\ifnotpaper
    ...
    (\Cref{qual-test}), requirements (\Cref{req-test}), and attacks
    (\Cref{attacks}). These are meaningful enough to merit analysis and are
    therefore in scope. Other test approaches given in the literature are
    derived from programming languages (\Cref{lang-test}) or other orthogonal
    test approaches (\Cref{orth-test}); these are out of scope as the
    information they provide is captured by other approaches.
