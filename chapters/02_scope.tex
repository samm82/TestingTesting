...
\begin{itemize}
    \item Ergonomics testing and proximity-based testing (see \citealpISTQB{})
          are out of scope.
          \ifnotpaper
    \item Similarly, \acf{emsec} testing (\citealp{ISO2021};
          \citealp[p.~95]{ZhouEtAl2012}), which deals with the ``security risk''
          of ``information leakage via electromagnetic emanation''
          \citep[p.~95]{ZhouEtAl2012}, is also out of scope.
    ...
    \item Conversely, the examples of environmental tolerance testing given by
          ...
          \fi
    \item \ifnotpaper Similarly, \fi \citet{SPICE2022} uses the terms ``software
