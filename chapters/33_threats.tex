...
\begin{itemize}
    \item \textbf{Single Researcher:} Since only one researcher was responsible
          for data collection, it was up to them to determine what data and
          relations were important to record and investigate. We mitigate this
          as much as possible by following a methodology (outlined in
          \Cref{methodology}) as rigorously as possible, including heuristics
    ...
    \item \textbf{Subjective Judgement Calls:} Because of the nuance of natural
          language and the fact that a single researcher collected\qtodo{
              Present tense?} our data (as
          mentioned before), we need to make some judgement calls on what
          data to include with which other researchers may disagree. In
          addition to the steps already mentioned, we mitigate this by
          outlining what we exclude from our scope in \Cref{app-scope}. This
          means that even if other researchers decide on a different scope, the
          data and results within the intersection of these scopes should
          match. Similarly, we document implicit information from the
    ...
    \item \textbf{Drift Over Time:} Since we began recording data
          ...
          \Cref{swebok-update})! While we could partially mitigate this by
          re-iterating over all sources with our ``finalized'' methodology,
          this is not possible due to time constraints. With respect to sources
          updating during our research, future researchers could replicate our
          results by reviewing a ``snapshot'' of them from when we reviewed%
          \qtodo{Present tense? And does this make sense?} them. However, if
          future researchers were to
          repeat this research, they \emph{should} use the most up-to-date
          sources and use these to update our existing data, methodology,
          and tools. This is one of the benefits of our research being
          open-source: it can exist as a ``living document'' that can (ideally)
          keep up with innovations to software testing!
\end{itemize}
