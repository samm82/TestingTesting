...
\citep[p.~154]{RunesonAndHöst2009}. The main objective of our research is to
document the issues with software testing terminology so that they can be
addressed in the future by improving relevant standards. Some hold that
``common terminology is dangerous'' and ``to be able to truly understand each
other, we need to ask questions and discuss in depth'' \citep{Schoots2014}.
However, it is possible to have a common terminology and still ``ask questions
and discuss in depth''---these are not mutually exclusive! Having a basic
``ground truth'' for what terms mean allows for common ground during these
sorts of discussions, along with the chance to adapt them to serve the context
of a given team or project (which we give examples of in
\Cref{syn-rels,alt-cats}).

Another concern about these standards in particular is that ``reality shows
standards become mandatory often'' \citep{Schoots2014}. This is supported by a blog
