...
\begin{itemize}
    \item \textbf{Similar Approach Categories:}
          ...
          Some sources \citep[such as][]{SWEBOK2024,BarbosaEtAl2006} propose
...
\begin{itemize}
    \item \phantomsection{}\label{flaw-def-threat}
          ...
          incomplete, inconsistent, and/or improperly coupled. When picking a
          ...
          which introduces some confusion \citetext{Dr.~R.~Paige, private
              communication, Oct.~14, 2025}. We attempt to mitigate this by
          being precise with our use of the words ``flaw'', ``error'',
          ``fault'', ``defect'', and ``failure''.
    \item \textbf{Manifestation and Domain:} Similarly, we define the terms
          ``manifestation'' and ``domain'' specifically in the context of how
          flaws appear in the literature. However, these terms can also be used
          in the context of software itself: a fault is a manifestation
          of a human error \citep[p.~278]{IEEE2017} and a domain is a
          ``distinct scope'' \citetext{p.~145\todo{OG ISO/IEC, 2012}} or ``problem
