...
While some of these synonyms may express mild variations in meaning, their core
...
Thus synonyms dot not inherently signify a discrepancy.
...
these terms. The following are \ifnotpaper\else some \fi examples that have
arisen\ifnotpaper\ (synonyms in \emph{italics} have at least one of their
      synonyms implied)\fi:
...
sub-approach of the other, abusing the meaning of \emph{synonym} and
...
\begin{itemize}
      ...
      \item \textbf{Conformance Testing} 
            ...
            %%% There are recommendations buried in this text. I think that
            % should be pulled out of here and put in the recommendation section.
...
``Recovery testing'', which is ``testing
...
From \citep{IEEE2022, IEEE2021, IEEE2017,
...
%%% Since there is a single one, I would actually not list it in the paper
\subsubsection{In Trusted Books}

These are from trusted books 
\citep{Patton2006, PetersAndPedrycz2000, vanVliet2000}, either internally or
between them, or between a book and a source from a previous ``category'':
