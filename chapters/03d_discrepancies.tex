...
found many discrepancies and ambiguities.
% We first report on the more
...
A summary of the number of different kinds of discrepancies is shown in
...
(i.e., a source from a category higher up in the table). Issues with
\nameref{syns}, \nameref{par-rels}, and \nameref{categories-discrep} are
(Exp)licit or (Imp)lied. \nameref{other-discrep}
are grouped into degrees of severity as follows. (Note that only certain
discrepancies with ``high'' or ``medium'' severity are listed for brevity.)
...
testing or functional testing'' \ifnotpaper \citet[p.~5-7]{SWEBOK2024}\else
      \cite[p.~5-7]{SWEBOK2024}\fi; this mirrors previous definitions
