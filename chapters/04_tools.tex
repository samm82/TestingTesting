...
\ifnotpaper
    ...
    the year 2022, and one in 2021. The following citation, ``(2022)'',
...
\ifnotpaper
    ...
    if its source is more ``credible'' (see \Cref{cred}). For example, if
    ``StdAuthor'' from \Cref{tab:synExampleGlossary} is an author of an
    established standard, than we display the implicit relation from their
    document alongside the explicit one from ``Author'' as shown in
    \Cref{fig:exampleSynGraph}. We denote the credibility of relations by
    colouring each source tier (defined in \Cref{source-tiers}) a different
    colour, which we label in the legend for each visualization.
    ...
    in \Cref{fig:exampleGraph,fig:exampleSynGraph}; explicit approaches are
...
subsets of them. \ifnotpaper For each approach category (see
    ...
    graphs grey %, as in \Cref{fig:staticExampleGraph}, 
    to distinguish them.
