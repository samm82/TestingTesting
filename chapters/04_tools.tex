...
result in something resembling a hierarchy (or multiple discrete hierarchies),
although this is not the case due to flaws in the literature (see
\Cref{selfParDef}). Therefore, we visualize all parent-child relations as they
are guaranteed to be meaningful.
...
\ifnotpaper
    ...
    visualization (such as \Cref{fig:exampleSynGraph}), although we omit this
...
When doing so\ifnotpaper\ in \Cref{recs}\fi, we colour any proposed approaches
