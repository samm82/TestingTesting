...
\ifnotpaper
    ...
    (see \Cref{explicitness}), we can represent explicit \emph{and} implicit
    ...
    (see \Cref{cred}). For example, if ``StdAuthor'' from
    \Cref{tab:synExampleGlossary} is an author of an established standard, then
    we display the implicit relation from their document alongside the explicit
    one from ``Author'' as shown in \Cref{fig:exampleSynGraph}.
    ...
    (except in the case of a more credible implicit relation we previously
