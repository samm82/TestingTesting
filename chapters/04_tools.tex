...
\ifnotpaper We can describe these graphs formally as ordered triplets
    $G = (A, S, P)$ comprising:
    \begin{itemize}
        \item $A$, a subset of the \approachCount{} test approaches we identify
              that function as the vertices of the graph,
        \item $S \subseteq \left\{ \{x, y\} \mid x, y \in V \wedge x \neq y \right\}$, a
              subset of our identified synonym relations (defined in
              \Cref{syn-rels}) that function as edges, and
        \item $P \subseteq \left\{(x,y) \mid (x, y) \in V^2 \right\}$, a subset of our
              identified parent-child relations (defined in \Cref{par-chd-rels})
              that also function as edges.
    \end{itemize}

    Since we use a consistent format to track synonym
    (see \Cref{syn-rels}) and parent-child relations (see \Cref{par-chd-rels})
...
\ifnotpaper
    ...
    \Cref{tab:synExampleGlossary}.
    \phantomsection{}\label{selfCycles}
    ...
    \phantomsection{}\label{graphRigid}
    Since we also track the ``rigidity'' of information (see \Cref{rigidity}),
    ...
    if its source is more ``trustworthy'' (see \Cref{trust}).
