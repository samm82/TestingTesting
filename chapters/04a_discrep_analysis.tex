...
generated by Python scripts in a similar fashion to calculated values. These
include the lists of sources in \Cref{sources}, which are built by extracting
all sources cited in our three glossaries, categorizing, sorting, and
formatting them, and saving them to a file to be saved as the macros
\macro{stdSources}, \macro{metaSources}, \macro{textSources}, and
...
repeated in different places, we manually create a new macro that accounts for
context-specific formatting (such as capitalization); these manual macros are
given in \Cref{tab:macrosText}, along with where they are used.
