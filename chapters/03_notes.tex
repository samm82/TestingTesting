...
BourqueAndFairley2014, istqb_glossary}.
\todo{should ``author names'' be acronyms or full?}

Each test is made of at least one ``approach'', defined as a ``high-level
test implementation choice, typically made as part of the test strategy
design activity'' \cite[p.~10]{IEEE2022}. This means that when looking at the
different kinds of testing, each one is called an ``approach''. The different
kinds of approach include ``test level, test type, test technique, test
practice and the form of static testing to be used'' \cite[p.~10]{IEEE2022},
this provides a way to classify these testing approaches as one (or more) of
the following kinds of approach:

\begin{itemize}
      \item \textbf{(Design) Technique:} ``procedure used to create or select a
            test model, identify test coverage items, and derive corresponding
            test cases'' (e.g., equivalence partitioning, boundary value
            analysis, branch testing) \cite[p.~11]{IEEE2022}
      \item \textbf{Level:} A stage of testing ``typically associated with the
            achievement of particular objectives and used to treat particular
            risks'' (e.g., unit/component testing, integration testing, system
            testing) \cite[p.~12]{IEEE2022}
      \item \textbf{Practice:} A ``conceptual framework that can be applied to
            \dots [a] test process to facilitate testing'' \cite[p.~14]{IEEE2022}
            (e.g., scripted testing, exploratory testing, automated testing)
            \cite[p.~20]{IEEE2022}
      \item \textbf{Type:} ``Testing that is focused on specific quality
            characteristics'' (e.g., functional testing, usability testing, and
            performance testing) \cite[p.~15]{IEEE2022}
\end{itemize}

From \cite{IEEE2022}, many types of testing were mentioned but not defined.
Additionally, there were some discrepancies or ambiguities (seven within just
this source and four between other sources); these may be areas for further
investigation:
...
\begin{enumerate}
      \item ``Compatibility testing'' is defined as ``testing that measures the
            ...
            ``interoperability'' being given in \cite{istqb_glossary}.
      ...
      \item ``Procedure testing'' is called a ``type of functional suitability
            testing'' \cite[p.~7]{IEEE2022}, but no definition of ``functional
            suitability testing'' is given; how does it relate to functional
            testing?
      ...
      \item ``Functional testing'' is given a loose definition of testing
            ``often used to check the implementation of functional
            requirements" \cite[p.~21]{IEEE2022}. While this source and
            \cite{istqb_glossary} define this and ``specification-based''
            testing as separate terms, \citet[p.~399]{vanVliet2000} lists them
            as synonyms.
...
\citep{BarbosaEtAl2006} gives the following: testing process, testing phase,
testing artifact, testing step, testing procedure, and testing resource
\citep[p.~2]{BarbosaEtAl2006},
as well as the relations between them \citep[Fig.~2]{BarbosaEtAl2006}
(see also \citep[Fig.~1]{BorgesAndBarbosa2009}, which I could only find as an
example of an ontology in an otherwise unrelated paper).
In addition to the concept of ``test artifact'', \citep{SouzaEtAl2017} also
provides the following concepts: testing technique, test level (which seems to
be comparable to ``testing phase'' from \citep[p.~3]{BarbosaEtAl2006}), and
test environment \citep[pp.~3-4]{SouzaEtAl2017}. 
Particular things of note from these ontologies:

\begin{itemize}
      \item The testing phases described by both are unit, integration,
            system, \citep[p.~3]{SouzaEtAl2017}, \citep[p.~3]{BarbosaEtAl2006},
            and regression testing \citep[p.~3]{BarbosaEtAl2006}
      ...
      \item ``Testing procedures can be categorized in testing methods,
            ...
            \citep[p.~3]{SouzaEtAl2017}) are examples of
            testing techniques  \citep[p.~3]{BarbosaEtAl2006}
\end{itemize}
...
\begin{itemize}
      \item Unit testing: ``testing the individual modules [of a program]''
            ...
            \citep[p.~444]{PetersAndPedrycz2000}. Note that since a
