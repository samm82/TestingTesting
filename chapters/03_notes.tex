...
\citealp[p.~5-13]{SWEBOK2024}; \citealp[p.~43]{Kam2008}), and
performance(-related) testing (\citealp[p.~38]{IEEE2021};
\citealp[p.~1187]{Moghadam2019}) may \emph{also}
...
There are many terms related to ``recovery testing'', which is the testing of
...
``Recovery testing'' is also given as a subset of performance-related testing
\citeyearpar[p.~22]{IEEE2022}, but this is not implied by the definition given
previously. It is also called ``recoverability testing'' \cite[p.~47]{Kam2008}
(which would make sense in the absence of performance considerations) and
...
are classified as test types (either explicitly or implicitly, like in the case
of ``backup and recovery testing''; this is inferred from its parent, reliability
testing \citeyearpar[p.~2]{IEEE2013}).

\begin{itemize}
      \item \textbf{Disaster/Recovery Testing:} Testing to determine the degree
            to which a system can ``return to normal operation after a hardware
      ...
      \item \textbf{Backup/Recovery Testing:} Testing that determines the
            ...
            (p.~37).
      \item \textbf{Failure/Recovery Testing:} Testing that determines the
            ...
            ``disaster/recovery testing'', even though that tests if ``operation
            of the test item can be transferred to a different operating site''
            (p.~37). Since this tests the actual backup process, it would make
            more sense for \emph{this} to include ``backup'' in its name.
\end{itemize}
