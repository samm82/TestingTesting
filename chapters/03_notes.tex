...
looking through textbooks that were trusted at McMaster \citep{patton_software_2006,
peters_software_2000, van_vliet_software_2000}. However, this process was
...
established sources of software testing terminology \citep{IEEE2022, IEEE2017,
SWEBOK2014, istqb_glossary}. \todo{should ``author names'' be acronyms or full?}
...
\dots of Test strategy'' \citep[p.~157]{tebes_systematic_2019}.

There are many different concepts involved with software testing;
\citep{barbosa_towards_2006} gives the following: testing process, testing phase,
testing artifact, testing step, testing procedure, and testing resource
\citep[p.~2]{barbosa_towards_2006},
as well as the relations between them \citep[Fig.~2]{barbosa_towards_2006}
(see also \citep[Fig.~1]{borges_using_2009}, which I could only find as an
example of an ontology in an otherwise unrelated paper).
In addition to the concept of ``test artifact'', \citep{souza_roost_2017} also
provides the following concepts: testing technique, test level (which seems to
be comparable to ``testing phase'' from \citep[p.~3]{barbosa_towards_2006}), and
test environment \citep[pp.~3-4]{souza_roost_2017}. 
Particular things of note from these ontologies:

\begin{itemize}
      \item The testing phases described by both are unit, integration,
            system, \citep[p.~3]{souza_roost_2017}, \citep[p.~3]{barbosa_towards_2006},
            and regression testing \citep[p.~3]{barbosa_towards_2006}
      \item Testing artifacts are ``produced and used throughout the testing
            process'' and include test plans, test procedures, test cases, and
            test results \citep[p.~3]{souza_roost_2017}. The role of testing
            artifacts is not specified in \citep{barbosa_towards_2006};
            requirements, drivers, and source code are all treated the same
            with no distinction \citep[p.~3]{barbosa_towards_2006}
      \item ``Testing procedures can be categorized in testing methods,
            testing guidances and testing techniques'', and ``functional,
            structural, error-based and state-based'' (called ``black-box''
            ``white-box'', ``defect-based'', and ``model-based'' in
            \citep[p.~3]{souza_roost_2017}) are examples of
            testing techniques  \citep[p.~3]{barbosa_towards_2006}
\end{itemize}

In \citep{souza_roost_2017}, the ontology (ROoST) \todo{add acronym?} is made to
...
\citep[pp.~8-9]{souza_roost_2017}. \todo{is this punctuation right?}
The question ``How do testing artifacts relate to each other?''
\citep[p.~8]{souza_roost_2017} is later broken down into multiple questions,
such as ``What are the test case inputs of a given test case?'' and ``What are
the expected results of a given test case?'' \citep[p.~21]{souza_roost_2017}.
...
definitions \citep{tebes_testtdo_terms_definitions_vfinalpdf_2020}
from TestTDO \citep{tebes_testtdo_2020} provides \emph{some} definitions of
...
to one another. \citep[pp.~152-153]{tebes_systematic_2019} may provide some
...
\citep[pp.~5-6]{perry_effective_2006}.

\citep{unterkalmsteiner_taxonomy_2014} provides a foundation to allow one ``to
...
[either domain]'' \citep[p.~A:2]{unterkalmsteiner_taxonomy_2014}.
...
\begin{itemize}
      \item Execution of code: static or dynamic
            \citep[p.~53]{patton_software_2006}
      \item Visibility of code: black-, white-, or gray-box (functional,
            structural, or a mix of the two)
            \citep[pp.~53, 218]{patton_software_2006}, \citep[p.~69]{perry_effective_2006}
      \item Stage of testing: unit, integration, system, or acceptance
            \citep{patton_software_2006}, \citep{perry_effective_2006},
            \citep{peters_software_2000} (sometimes includes installation
            \citep[p.~439]{van_vliet_software_2000} or regression
            \citep[p.~3]{barbosa_towards_2006})
      ...
      \item Source of test data: specification-, implementation-, or
            error-oriented \citep[p.~440]{peters_software_2000}
...
\citep[p.~40]{perry_effective_2006}. These include correctness, file integrity,
...
training) \citep[pp.~40-41]{perry_effective_2006}. \emph{These may overlap with
...
\begin{itemize}
      ...
      \item Fault: ``the manifestation of an error'' in the software itself
            \citep[p.~400]{van_vliet_software_2000}
      ...
      \item Test Suite Reduction: the process of reducing the size of a test
            ...
            \citep[p.~519]{barr_oracle_2015}; can be accomplished through
      ...
      \item Test Case Reduction: the process of ``removing side-effect free
            ...
            costs'' \citep[p.~519]{barr_oracle_2015}
...
\begin{itemize}
      \item \acf{vnv} Plan: a document for the ``planning of test activities''
            described by IEEE Standard 1012 \citep[p.~411]{van_vliet_software_2000}
      ...
      \item Test Procedure documentation: ``specifies the sequence of actions
            for the execution of each test'' \citep[p.~413]{van_vliet_software_2000}
      \item Test Report documentation: ``provides information on the results of
            ...
            reporting \citep[p.~413]{van_vliet_software_2000}
\end{itemize}
...
\begin{itemize}
      \item ``Proving the correctness of software \dots applies only in
            ...
            \citep[p.~398]{van_vliet_software_2000}
      \item If faults exist in programs, they ``must be considered faulty, even
            ...
            \citep[p.~401]{van_vliet_software_2000}
      \item Black-box test cases should be created based on the specification
            ...
            \citep[p.~113]{patton_software_2006}
      \item Simple, normal test cases (test-to-pass) should always be developed
            ...
            \citep[p.~66]{patton_software_2006}
      \item Since ``there is no uniform best test technique'', it is advised to
            use many techniques when testing \citep[p.~440]{van_vliet_software_2000}
      \item When comparing adequacy criteria, ``criterion X is stronger than
            ...
            the ``subsumes'' relation) \citep[p.~432]{van_vliet_software_2000};
            this relation only ``compares the thoroughness of test techniques,
            not their ability to detect faults'' \citep[p.~434]{van_vliet_software_2000}
            \todo{This should probably be explained after ``test adequacy
                  criterion'' is defined}
\end{itemize}

\subsubsection{Steps to Testing \citep[p.~443]{peters_software_2000}}
\begin{enumerate}
...
\begin{itemize}
      \item Unit testing: ``testing the individual modules [of a program]''
            \citep[p.~438]{van_vliet_software_2000}; also called ``module testing''
            \citep[p.~109]{patton_software_2006} or ``component testing''
            \citep[p.~444]{peters_software_2000}. Note that since a
            \emph{component} is ``a part of a system that can be tested in
            isolation'' \citep{istqb_glossary}, this seems like it could apply
            ...
            \begin{itemize}
                  ...
                  \item Big bang testing: the process of ``integrat[ing] all
                        ...
                        \emph{Although this is ``quite challenging and risky''
                              ...
                              ``to plan and control''
                              \citep[p.~490]{peters_software_2000}
                              \qtodo{Bring up!}}
                  \item Sandwich testing: ``combines the ideas of bottom-up and
                        ...
                        \citep[p.~491]{peters_software_2000}
            \end{itemize}
      \item System testing: ``test[ing] the whole system against the user
            ...
            testing has finished'' \citep[p.~439]{van_vliet_software_2000}
            (\citep[p.~109]{patton_software_2006} says this can also be done on
      ...
      \item Acceptance testing: Similar to system testing that is ``often
            ...
            focusing on usability \citep[p.~439]{van_vliet_software_2000} and
            the needs of the customer(s) \citep[p.~492]{peters_software_2000}
      \item Installation testing: Focuses on the portability of the product,
            ...
            has been developed'' \citep[p.~439]{van_vliet_software_2000}; not
...
not'' \citep[p.~509]{barr_oracle_2015}. They can either be ``deterministic''
(returning a Boolean value) or ``probabilistic'' (returning ``a real number in
the closed interval [0, 1]'') \citep[p.~509]{barr_oracle_2015}. Probabilistic
...
\begin{itemize}
      ...
      \item Derived test oracle: any ``artefact[] from which a
            ...
            \citep[p.~510]{barr_oracle_2015}, and invariant detection (either
            known in advance or ``learned from the program'')
            \citep[p.~516]{barr_oracle_2015}; \emph{This is like the assertions
      ...
      \item Pseudo-oracle: a type of derived test oracle that is ``an
            ...
            \citep[p.~515]{barr_oracle_2015}. \emph{We could potentially use the
...
\begin{itemize}
      \item ``A \textbf{test adequacy criterion} \dots specifies requirements
            ...
            statements yet untested'' \citep[p.~402]{van_vliet_software_2000}
      \item ``Test data generators'' are mentioned on
            \citep[p.~410]{van_vliet_software_2000} but not described
            \todo{Investigate}
\end{itemize}

\subsection{Static Black-Box (Specification) Testing
      \citep[pp.~56-62]{patton_software_2006}}
...
\citep[pp.~57-58]{patton_software_2006}, Research Existing Standards and
Guidelines \citep[pp.~58-59]{patton_software_2006}). However, it provides a
``Specification Terminology Checklist'' \citep[p.~61]{patton_software_2006} that
...
\begin{itemize}
      ...
      \item \textbf{Potentially incomplete:} etc., and so forth, and so on,
            ...
            to be determined \citep[p.~408]{van_vliet_software_2000}
\end{itemize}

\subsubsection{Coverage-Based Testing of Specification
      \citep[pp.~425-426]{van_vliet_software_2000}}
...
cases can be derived \citep[p.~425]{van_vliet_software_2000}. However, it can
...
\subsubsection{Exploratory Testing \citep[p.~65]{patton_software_2006}}

An alternative to dynamic black-box testing when a specification is not
available \citep[p.~65]{patton_software_2006}. The software is explored to
determine its features, and these features are then tested
\citep[p.~65]{patton_software_2006}. Finding any bugs using this method is a
positive thing \citep[p.~65]{patton_software_2006}, since despite not knowing
...
\subsubsection{Equivalence Partitioning/Classing \citep[pp.~67-69]{patton_software_2006}}

The process of dividing the infinite set of test cases into a finite set that is
just as effective (i.e., that reveals the same bugs) \citep[p.~67]{patton_software_2006}.
The opposite of this, testing every combination of inputs, is called
``exhaustive testing'' and is ``probably not feasible''
\citep[p.~461]{peters_software_2000}.

\paragraph{Requirements}
\begin{itemize}
      \item Ranges of possible values \citep[p.~67]{patton_software_2006};
...
\subsubsection{Data Testing \citep[pp.~70-79]{patton_software_2006}}

The process of ``checking that information the user inputs [and] results'',
both final and intermediate, ``are handled correctly''
\citep[p.~70]{patton_software_2006}. This type of testing can also occur at the
...
\paragraph{Boundary Conditions \citep[pp.~70-74]{patton_software_2006}}

``[S]ituations at the edge of the planned operational limits of the software''
\citep[p.~72]{patton_software_2006}. Often affects types of data (e.g., numeric,
speed, character, location, position, size, quantity
\citep[p.~72]{patton_software_2006}) each with its own set of (e.g., first/last,
...
next-to/farthest-from \citep[pp.~72-73]{patton_software_2006}). Data at these
boundaries should be included in an equivalence partition, but so should
data in between them \citep[p.~73]{patton_software_2006}. Boundary conditions
...
boundary'' \citep[p.~73]{patton_software_2006}, and values at the boundaries
...
testing may reveal it \citep[p.~460]{peters_software_2000}.

\subparagraph{Requirements}
\begin{itemize}
      \item Ranges of possible values \citep[p.~67, 73]{patton_software_2006};
            could be obtained through:
            \begin{itemize}
                  \item Case statements
...
\subparagraph{Buffer Overruns \citep[pp.~201-205]{patton_software_2006}}

\emph{Buffer overruns} are ``the number one cause of software security issues''
\citep[p.~75]{patton_software_2006}. They occur when the size of the destination
...
\citep[p.~202, 204-205]{patton_software_2006}. They often arise from bad
programming practices in ``languages [sic] such as C and C++, that lack safe
string handling functions'' \citep[p.~201]{patton_software_2006}. Any unsafe
versions of these functions that are used should be replaced with the
corresponding safe versions \citep[pp.~203-204]{patton_software_2006}.

\paragraph{Sub-Boundary Conditions \citep[pp.~75-77]{patton_software_2006}}
\label{sub-bound-conds}

Boundary conditions ``that are internal to the software [but] aren't necessarily
apparent to an end user'' \citep[p.~75]{patton_software_2006}. These include
powers of two \citep[pp.~75-76]{patton_software_2006} and ASCII and Unicode tables
\citep[pp.~76-77]{patton_software_2006}.
...
\citep[p.~430]{van_vliet_software_2000}. These ``error prone'' points around
these boundaries should be tested \citep[p.~430]{van_vliet_software_2000} as
...
\citep[p.~73]{patton_software_2006}. In this type of testing, the second type of
...
\citep[p.~430]{van_vliet_software_2000}.
...
\paragraph{Default, Empty, Blank, Null, Zero, and None
      \citep[pp.~77-78]{patton_software_2006}}
...
\citep[p.~78]{patton_software_2006}.
...
\paragraph{Invalid, Wrong, Incorrect, and Garbage Data
      \citep[pp.~78-79]{patton_software_2006}}

This is testing-to-fail \citep[p.~77]{patton_software_2006}.
...
\paragraph{Syntax-Driven Testing \citep[pp.~448-449]{peters_software_2000}}

If the inputs to the system ``are described by a certain grammar''
\citep[p.~448]{peters_software_2000}, ``test cases \dots [can] be designed
according to the syntax or constraint of input domains defined in requirement
specification'' \citep[p.~260]{intana_syntest_2020} 
\todo{Investigate this source more!}.

\paragraph{Decision Table-Based Testing \citep[pp.~448,450-453]{peters_software_2000}}
...
test situation \citep[p.~448]{peters_software_2000}. ``The upper part of the
...
\subsubsection{State Testing \citep[pp.~79-87]{patton_software_2006}}

The process of testing ``the program's logic flow through its various states''
\citep[p.~79]{patton_software_2006} by checking that state variables are
...
\citep[pp.~81-82]{patton_software_2006}. Next, these states should be
...
\citep{istqb_glossary}. \todo{Original source: ISO 25010?} 

% ``The intent of this type of testing is to identify weak points of a software
% system and quantify its shortcomings'' \citep[p.~447]{peters_software_2000}.

\todo{Originally used a \emph{very} vague definition from
\citep[p.~447]{peters_software_2000}; re-investigate!}

\paragraph{Testing States to Fail \citep[pp.~84-87]{patton_software_2006}}

The goal here is to try and put the program in a fail state by doing things
that are out of the ordinary. These include:

\begin{itemize}
      \item Race Conditions and Bad Timing \citep[pp.~85-86]{patton_software_2006}
      ...
      \item Repetition Testing: ``doing the same operation over and over'',
            ...
            \citep[p.~86]{patton_software_2006}
      \item Stress Testing: ``running the software under less-than-ideal
            conditions'' to ``determine what external resources and
            dependencies it has'' \citep[p.~86]{patton_software_2006}
...
\subsubsection{Other Black-Box Testing \citep[pp.~87-89]{patton_software_2006}}
\begin{itemize}
...
\subsection{Static White-Box Testing (Structural Analysis)
      \citep[pp.~91-104]{patton_software_2006}}

White-box testing is also called ``glass box testing''
\citep[p.~439]{peters_software_2000}. \citep[p.~447]{peters_software_2000} claims
...
Evaluation \citep[pp.~417-418]{van_vliet_software_2000} and Stepwise Abstraction
\citep[pp.~419-420]{van_vliet_software_2000}, that could be investigated further.
\todo{Do this!}
...
\begin{itemize}
      \item ``The process of carefully and methodically reviewing the software
            ...
            \citep[p.~92]{patton_software_2006}
      \item Less common than black-box testing, but often used for ``military,
            ...
            for security issues'' \citep[p.~91]{patton_software_2006}; often
      ...
      \item Largely ``done by the language compiler'' or by separate tools
            \citep[pp.~413-414]{van_vliet_software_2000}
\end{itemize}

\subsubsection{Reviews \citep[pp.~92-95]{patton_software_2006},
      \citep[pp.~415-417]{van_vliet_software_2000},
      \citep[pp.~482-485]{peters_software_2000}}
\label{reviews}

\begin{itemize}
      \item ``The process under which static white-box testing is performed''
            \citep[p.~92]{patton_software_2006}; consists of four main parts:
            ...
            \begin{enumerate}
                  ...
                  \item Write a Report: A summary should be created and provided
                        ...
                        \citep[p.~93]{patton_software_2006}
            \end{enumerate}
      ...
      \item Many forms:
            \begin{itemize}
                  \item Peer Review: Also called ``buddy review''
                        \citep[p.~94]{patton_software_2006}. The most informal
                        review at the smallest scale \citep[p.~94]{patton_software_2006}.
                        One variation is where a group of two or three people
                        go through code that one of them wrote
                        \citep[p.~94]{patton_software_2006}. Another is to
...
\subsubsection{Coding Standards and Guidelines \citep[pp.~96-99]{patton_software_2006}}
...
\begin{itemize}
      \item Code may work but still be incorrect if it doesn't meet certain
            ...
            if used incorrectly \citep[p.~96]{patton_software_2006}
      \item These guidelines can range in strictness and formality, as long as
            they are agreed upon and followed \citep[p.~96]{patton_software_2006}
      \item This could be checked using linters
\end{itemize}

\subsubsection{Generic Code Review Checklist \citep[pp.~99-103]{patton_software_2006}}
...
\subsubsection{Correctness Proofs \citep[pp.~418-419]{van_vliet_software_2000}}
Requires a formal specification \citep[p.~418]{van_vliet_software_2000} and uses
``highly formal methods of logic'' \citep[p.~438]{peters_software_2000} to prove
...
``sometimes disputed'' \citep[p.~418]{van_vliet_software_2000}.
...
\subsection{Dynamic White-Box (Structural) Testing
      \citep[pp.~105-121]{patton_software_2006}}
...
\citep[p.~106]{patton_software_2006}.

\subsubsection{Code Coverage \citep[pp.~117-121]{patton_software_2006} or
      Control-Flow Coverage \citep[pp.~421-424]{van_vliet_software_2000}}

``[T]est[ing] the program's states and the program's flow among them''
\citep[p.~117]{patton_software_2006}; allows for redundant and/or missing test
cases to be identified \citep[p.~118]{patton_software_2006}. Coverage-based
...
to loops \dots'' \citep[pp.~420-421]{van_vliet_software_2000}. ``A cycle is
called \emph{simple} if its inner nodes are distinct and do not include
      [the node at the beginning/end of the cycle]''
\citep[p.~421, emphasis added]{van_vliet_software_2000}. If there are
multiple actions represented as nodes that occur one after another, they may
be collapsed into a single node \citep[p.~421]{van_vliet_software_2000}.
...
coverage \citep[p.~421]{van_vliet_software_2000}).

\begin{itemize}
      \item Statement/line coverage: attempting to ``execute every statement in
            the program at least once'' \citep[p.~119]{patton_software_2006}
            \begin{itemize}
                  \item Weaker than \citep[p.~421]{van_vliet_software_2000} and
                        ``only about 50\% as effective as branch coverage''
                        \citep[p.~481]{peters_software_2000}
                  ...
                  \item ``[C]an be used at the module level with less than 5000
                        lines of code'' \citep[p.~481]{peters_software_2000}
                        \todo{Find original source: Miller et al., 1994}
                  \item Doesn't guarantee correctness
                        \citep[p.~421]{van_vliet_software_2000}
            \end{itemize}
      \item Branch coverage: attempting to, ``at each branching node in the
            ...
            once'' \citep[p.~421]{van_vliet_software_2000}
            \begin{itemize}
                  \item Weaker than path coverage
                        \citep[p.~433]{van_vliet_software_2000}, although
                        \citep[p.~119]{patton_software_2006} says it is
      ...
      \item Path coverage: ``[a]ttempting to cover all the paths in the
            ...
            \citep[p.~420]{van_vliet_software_2000})
            ...
            \begin{itemize}
                  \item The number of paths to test can be bounded based on its
                        ...
                        individually \citep[pp.~471-473]{peters_software_2000};
                        this is less feasible if a loop is present
                        \citep[pp.~473-476]{peters_software_2000} since ``a loop
      ...
      \item (Multiple) condition coverage: ``takes the extra conditions on the
            ...
            Boolean expression) \citep[p.~120]{patton_software_2006}
            \begin{itemize}
                  \item ``Also known as \textbf{extended branch coverage}''
                        \citep[p.~422]{van_vliet_software_2000}
                  ...
                  \item ``May be quite challenging'' since ``if each
                        ...
                        \citep[p.~464]{peters_software_2000}
            \end{itemize}
\end{itemize}

\subsubsection{Data Coverage \citep[pp.~114-116]{patton_software_2006}}
...
\begin{itemize}
      ...
      \item Error forcing: setting variables to specific values to see how
            ...
            \citep[p.~116]{patton_software_2006}
\end{itemize}

\paragraph{Data Flow Coverage \citep[p.~114]{patton_software_2006},
      \citep[pp.~424-425]{van_vliet_software_2000}}
...
\begin{itemize}
      ...
      \item ``A definition in statement X is \emph{alive} in statement Y if
            ...
            \citep[p.~424]{van_vliet_software_2000}
      \item A path from a variable's definition to a statement where it is
            ...
            this variable) \citep[p.~424]{van_vliet_software_2000}
      \item Basic block: ``[a] consecutive part[] of code that execute[s]
            together without any branching'' \citep[p.~477]{peters_software_2000}
...
\citep[pp.~424-425]{van_vliet_software_2000} \todo{Is this sufficient?}.
...
\subsubsection{Fault Seeding \citep[pp.~427-428]{van_vliet_software_2000}}
...
\citep[p.~427]{van_vliet_software_2000}. Makes many assumptions, including
...
\citep[p.~427]{van_vliet_software_2000}.

\subsubsection{Mutation Testing \citep[pp.~428-429]{van_vliet_software_2000}}
...
operator with another) \citep[p.~428]{van_vliet_software_2000}. These
...
quality'' \citep[pp.~428-429]{van_vliet_software_2000}. The goal is to maximize
the number of mutants identified by a given test set
\citep[p.~429]{van_vliet_software_2000}. \textbf{Strong mutation testing} works
at the program level while \textbf{weak mutation testing} works at the
component level (and ``is often easier to establish'')
\citep[p.~429]{van_vliet_software_2000}.
...
\citep[p.~432]{van_vliet_software_2000}! In addition to revealing shortcomings
of a test set, mutation testing can also point the developer(s) in the
direction of a better solution!

\subsection{Gray-Box Testing \citep[pp.~218-220]{patton_software_2006}}
...
at what makes the software work'' \citep[p.~218]{patton_software_2006}. An
...
tedious \citetext{p.~482}). See also \citep[p.~232]{patton_software_2006}.

\begin{itemize}
      \item Should be done automatically \citep[p.~481]{peters_software_2000};
            ...
            \emph{oracles}'' \citep[p.~516]{barr_oracle_2015}
      \item Its ``effectiveness \dots is expressed in terms of'':
            \begin{enumerate}
                  ...
                  \item reliability of the testing system
                        \citep[pp.~481-482]{peters_software_2000}
            \end{enumerate}
      \item Various levels:
            \begin{itemize}
                  \item Retest-all: ``all tests are rerun''; ``this may consume
                        a lot of time and effort''
                        \citep[p.~411]{van_vliet_software_2000} (\emph{shouldn't
                  ...
                  \item Selective retest: ``only some of the tests are rerun''
                        ...
                        implemented yet'' \citep[p.~411]{van_vliet_software_2000}
...
\citep[p.~67]{kanewala_metamorphic_2019}. ``A[n] \acs{mr} specifies how the
output of the program is expected to change when a specified change is made to
the input'' \citep[p.~67]{kanewala_metamorphic_2019}; this is commonly done by
...
should both pass) \citep[p.~68]{kanewala_metamorphic_2019}. ``\acs{mt} is one of
the most appropriate and cost-effective testing techniques for scientists and
engineers'' \citep[p.~72]{kanewala_metamorphic_2019}.

\subsubsection{Benefits of \acs{mt}}
\begin{itemize}
      \item Easier for domain experts; not only do they understand the domain
            (and its relevant \acp{mr}) \citep[p.~70]{kanewala_metamorphic_2019},
            they also may not have an understanding of testing principles
            \citep[p.~69]{kanewala_metamorphic_2019}. \emph{This majorly
                  overlaps with Drasil!}
      \item Easy to implement via scripts \citep[p.~69]{kanewala_metamorphic_2019}.
            \emph{Again, Drasil}
      \item Helps negate the test oracle \citep[p.~69]{kanewala_metamorphic_2019}
            and output validation \citep[p.~70]{kanewala_metamorphic_2019} problems
      ...
      \item Can extend a limited number of test cases (e.g., from an
            ...
            \citep[pp.~70-72]{kanewala_metamorphic_2019}
      \item Domain experts are sometimes unable to identify faults in a program
            based on its output \citep[p.~71]{kanewala_metamorphic_2019}
\end{itemize}
...
\begin{itemize}
      \item The average of a list of numbers should be equal (within
            ...
            \citep[p.~67]{kanewala_metamorphic_2019}
      \item For matrices, if $B = B_1 + B_2$, then $A \times B = A \times B_1
                  + A \times B_2$ \citep[pp.~68-69]{kanewala_metamorphic_2019}
      \item Symmetry of trigonometric functions; for example, $\sin(x) = \sin(-x)$
            and $\sin(x) = \sin(x + 360^{\circ})$ \citep[p.~70]{kanewala_metamorphic_2019}
      \item Modifying input parameters to observe expected changes to a model's
            ...
            \citep[p.~70]{kanewala_metamorphic_2019}
      \item Using machine learning to predict likely \acsp{mr} to identify
            ...
            \citep[p.~71]{kanewala_metamorphic_2019}
\end{itemize}
...
\begin{itemize}
      \item Intractability: it is generally impossible to test a program
            exhaustively \citep[p.~421]{van_vliet_software_2000},
            \citep[pp.~439,461]{peters_software_2000}
      \item Undecidability \citep[p.~439]{peters_software_2000}: it is
...
\begin{itemize}
      \item ``Correct answers are often unknown'': if the results were already
            ...
            \citep[p.~67]{kanewala_metamorphic_2019}; in other words, complete
      ...
      \item ``Inherent uncertainties'': since scientific software models
            ...
            factor can be accounted for \citep[p.~67]{kanewala_metamorphic_2019}
      ...
      \item ``Incompatible testing tools'': while scientific software is often
            ...
            int languages like Java or C++ \citep[p.~67]{kanewala_metamorphic_2019}
\end{itemize}
