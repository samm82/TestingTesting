...
For example, ``error seeding'' is the ``process of intentionally adding known
faults to those already in a computer program \dots [to] monitor[] the rate of
detection and removal[] and estimat[e] the number of faults remaining''
\citep[p.~165]{IEEE2017}. While ``monitoring the rate of [fault] detection and
removal'' is a part of \acs{vnv} of the \acs{vnv} itself, ``estimating the
number of faults remaining'' \citep[p.~165]{IEEE2017} helps verify the actual
code.

Sometimes, the term ``testing'' excludes static testing, as done by
\citet[p.~5-1]{SWEBOK2024}. Since ``terminology is not uniform among
different communities, and some use the term \emph{testing} to refer to static
techniques as well'' \cite[p.~5-2]{SWEBOK2024}, the broad term ``testing''
will include both ``static testing'' and ``dynamic testing'' throughout this
project, as done by \citet[p.~17]{IEEE2022}. The end goal of this project is to
be able to generate test cases automatically, which will not be possible for
certain testing approaches (like those that require human interaction; e.g.,
as A/B testing, audits, and beta testing) and will not be feasible within a
reasonable time frame for others. However, understanding the breadth of testing
approaches provides a more complete picture of how software can be tested,
how the various approaches are related to one another, and potentially
how even parts of these ``out-of-scope'' approaches may be generated in the
future! These ``out-of-scope'' approaches will be identified more
systematically, but gathering information about them is an important precursor,
making them within the scope of this research.

\subsection{Methodology}

This process initially involved
...
established sources of software testing terminology in approximately the
following order:
\citep{IEEE2022, SWEBOK2024, SWEBOK2014, IEEE2017, IEEE2013, ISO_IEC2023b,
      IEEE2012, ISO_IEC2023a, ISTQB}.
...
glossary to indicate it should be investigated further. Since testing types can
be derived from software qualities \todo{cite discussion/source}, these were
also tracked following a similar methodology, albeit in a separate document.
...
\begin{enumerate}
      ...
      \item SWEBOK V4 says ``scalability testing evaluates the capability to
            ...
            (\citeauthor{ISTQB} does this explicitly):
            \begin{itemize}
                  ...
                  \item ``The degree to which a component or system can be
                        adjusted for changing'' \citep{ISTQB}
            \end{itemize}
...
\citealp{ISTQB}) while some refer to ``operational testing''
...
acceptance testing (\citealp[p.~22]{IEEE2022}; \citealp{ISTQB}) that focuses on
