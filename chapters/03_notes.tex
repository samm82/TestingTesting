...
\begin{itemize}
      ...
      \item ``Generators are often the technique of last resort \dots to
            automate, elevate, modularize or otherwise facilitate program
            development'' \cite[p.~2]{smaragdakis_structured_2017}
            \todo{Can I rearrange quotes like this?}
...
\subsection{Roadblocks to Meta-Programming}
\begin{itemize}
...
\subsection{Difficulties in Testing Scientific Software
...
\begin{itemize}
      ...
      \item Two approaches for testing incrementally:
            \begin{itemize}
                  \item Bottom-up testing: ``you write your own modules, called
                        \emph{test drivers}, that exercise the modules you're
                        testing \dots [by] sending test-case data to the modules
                        under test, read[ing] back the results, and verify[ing]
                        that they're correct'' \cite[p.~109]{patton_software_2006}
                  \item Top-down testing: uses \emph{stubs} (pieces of code that
                        provide ``fake'' values to a given module)
                        \cite[p.~110]{patton_software_2006}
            \end{itemize}
      \item Black-box test cases should be created based on the specification
            \emph{before} creating white-box test cases to avoid being ``biased into
            creating test cases based on how the module works''
...
\begin{itemize}
      ...
      \item Path coverage: ``[a]ttempting to cover all the paths in the
            software''; \emph{branch coverage} is the simplest form (but the
