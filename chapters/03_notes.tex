...
\begin{itemize}
      \item Easier for domain experts; not only do they understand the domain
            (and its relevant \acp{mr}), they also may not have an
            understanding of testing principles. \emph{This majorly overlaps
                  with Drasil!}
      \item Easy to implement via scripts. \emph{Again, Drasil}
      \item Helps negate the test oracle and output validation problems from
            \nameref{chap:notes:sec:sci-testing-roadblocks} (\emph{i.e., the
                  two that are relevant for Drasil})
