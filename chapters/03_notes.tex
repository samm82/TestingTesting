...
\thesisissueref{57}). The following terms (and their respective related terms)
...
\begin{itemize}
      ...
      \item \textbf{EMSEC Testing:} \citet{ZhouEtAl2012, ISO2021}\footnote{
                  ...
                  \nameref{chap:testing:sec:scope}), these sources were not
                  fully investigated.}
...
As an extreme example of this, it originally
...
and re-establish[ing] the desired state of the system'' \citep{ISO_IEC2023a} so
...
\begin{itemize}
      \item \textbf{Recoverability Testing:} Testing ``how well a system or
            ...
            (\citealp[p.~7-10]{SWEBOK2024}; similar in \citealp[p.~369]{IEEE2017}
            \todo{OG ISO/IEC, 2011}) and ``re-establish the desired state of the
            system'' (p.~369) \todo{OG ISO/IEC, 2011}.
            %\citep[p.~369]{IEEE2017}
...
Additionally, there seemed to be a distinction between generic ``recovery
testing'' and its performance aspects, as discussed in \thesisissueref{40}.
I proposed two new divisions to capture these differences in the literature:
the term ``recoverability testing'' should be used to denote a system's ability
to recovery (the use of this term is supported by \citet[p.~47]{Kam2008}), and
``recovery performance testing'', which focuses on the performance-related
aspects (see \nameref{perf-test-ambiguity}) of a system. The following details
the elements of ``recovery testing'' that are specific to performance:

\begin{itemize}
      \item ``Recovery testing'' is given as a type of performance-related
            testing \citep[p.~22]{IEEE2022}, which is not implied by its
            given definitions.
      \item ``Backup and recovery testing'' occurs ``within specified
            parameters of time, cost, completeness, and accuracy''
            \citep[p.~2]{IEEE2013}.
\end{itemize}

\begin{figure}[hbtp!]
      \centering
      \caption{The current state of the relations between
