...
\Cref{fig:recovery-graph-current,fig:recovery-graph-proposed,%
    fig:scal-graph-current,fig:scal-graph-proposed,fig:perf-graph}, in which
arrows representing relations between approaches are coloured based on the
source tier (see \Cref{sources}) that defines them.
