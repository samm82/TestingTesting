...
flaws \emph{themselves}. The exceptions to this are \Cref{tab:flawTable,%
    fig:flawBars}, which count the following sources of flaws separately:
\begin{enumerate}
...
counted flaws possible within a \emph{single} source tier in \Cref{tab:flawTable,%
    fig:flawBars} is four (one for each type). This only occurs if
...
\begin{figure}[bt!]
    ...
    \begin{tikzpicture}
        ...
        \node[above] at (0, -\spread-0.5) {\parbox{3.7cm}{\centering
                \citealp{IEEE2022,IEEE2021,IEEE2019,IEEE2017,IEEE2016,
                    IEEE2013,IEEE2010}}};
        % ISO/IEC
...
\begin{itemize}
    \item \textbf{\stds{}}: this flaw occurs:
          ...
          given in \Cref{tab:flawTable,fig:flawBars} and explained in more detail in
...
which row(s) and column(s) of \Cref{tab:flawTable},
...
\Cref{tab:flawTable,fig:flawBars}, but only once to avoid double counting.
...
\Cref{tab:flawTable,fig:flawBars} due to their more nuanced breakdown of the
sources of flaws. The rest increment their corresponding count in
\Cref{tab:flawTable,fig:flawBars,tab:flawMnfsts,tab:flawDmns} by only one:
\begin{itemize}
