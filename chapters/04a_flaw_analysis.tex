...
\begin{itemize}
    ...
    \item how obvious (or ``explicit''; see \Cref{explicitness}) these flaws are,
...
entries such as those in \Cref{tab:synExampleGlossary}. We automatically detect
and format these flaws to present them when discussing synonym relation flaws
in \Cref{syns}. % \Cref{multiSyns,infMultiSyns}
...
should not be double counted as both\thesisissueref{83,176}! When counting
flaws in \Cref{tab:flawMnfsts,tab:flawDmns}, each one is counted only for its
