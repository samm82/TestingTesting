...
information alongside each flaw (\Cref{flaw-comment-syntax}), both when
detecting flaws automatically where applicable (\Cref{auto-flaw-detect}) and
when recording flaws manually, so we can parse and analyze this information
using automated tools (\Cref{flaw-comment-analysis}). This gives us an overview of:
\begin{itemize}
...
\subsubsection{Flaw Comment Syntax}\label{flaw-comment-syntax}
