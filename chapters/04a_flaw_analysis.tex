...
\begin{displayquote}
    \texttt{\% Flaw count (SEM, SYN): \{A1\} \{A2\} \dots{} | \{B1\} \{B2\}
        \dots{} | \{C1\} \dots}
\end{displayquote}
\texttt{SEM} and \texttt{SYN} are placeholders for ``codes'' we use to identify
a flaw's semantics and syntax (see \Cref{flaws}). Each flaw has exactly one of
each, and their values are given in \Cref{tab:flawCodes}\latertodo{Update
    references to flaw sections once their format is locked down; see
    \thesisissuerefhelper{134}}. We omit these codes
from the examples of these comments throughout this chapter for brevity.
\begin{table*}[b]
    \centering
    \begin{talltblr}[
        % note{a} = {Section omitted for brevity.},
        caption = {``Codes'' used to indicate a flaw's semantics and syntax.},
        label = {tab:flawCodes}
        ]{
        colspec={|Q[c,m]Q[c,m]||Q[c,m]Q[c,m]|},
        column{1} = {font=\ttfamily}, column{3} = {font=\ttfamily},
        width = \textwidth, rowhead = 1
        }
        \hline
        \thead{SEM} & \thead{Semantic Flaws} & \thead{SYN} & \thead{Syntactic Flaws} \\
        \hline
        CATS        & \nameref{cats}         & WRONG       & \nameref{wrong}         \\
        SYNS        & \nameref{syns}         & MISS        & \nameref{miss}          \\
        PARS        & \nameref{pars}         & CONTRA      & \nameref{contra}        \\
    ...
    \end{talltblr}
\end{table*}
Finally, \texttt{A1}, \texttt{A2}, \texttt{B1},
...
\begin{displayquote}
    \texttt{\% Flaw count: \{X\}\\\% Flaw count: \{X\} | \{X\}\\
        \% Flaw count: \{X\} | \{X\} \{Y\}}
\end{displayquote}
We do not double count discrepancies that reappear when comparing between pairs
of groups; this means the following line adds discrepancies between X and Z
\emph{and} between Y and Z without double counting the discrepancy between X and Z.
\begin{displayquote}
...
this as a discrepancy between documents by the same author(s) in
