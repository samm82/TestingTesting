...
\Cref{tab:flawMnfsts,tab:smntcFlaws} in the relevant column:
...
to determine the row(s) of \Cref{tab:flawMnfsts,tab:smntcFlaws} and the graph(s)
...
\Cref{tab:flawMnfsts,tab:smntcFlaws}, each one is
...
\Cref{tab:flawMnfstDefs,tab:smntcFlawKinds} that we use to identify
...
\Cref{fig:flawSources,tab:flawMnfsts,tab:smntcFlaws} by only one:
\begin{itemize}
...
to populate \Cref{tab:flawMnfsts,tab:smntcFlaws}. For example,
