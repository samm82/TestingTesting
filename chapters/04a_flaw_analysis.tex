...
credible tier\footnote{If an inconsistency occurs between two source tiers
    ...
    source-tiers}). This avoids counting the same flaw more than once for a
given source tier, which would give the % \thesisissuetodo{83}
...
\begin{enumerate}
    ...
    \item those between documents with the same set of authors, which includes:
          \begin{enumerate}
              ...
              \item the different versions of the \acfp{swebok}
                    \citep{SWEBOK2025,SWEBOK2024,SWEBOK2014}\footnote{Although
                        these documents have different editors, they are
                        published by the same organization: the IEEE Computer
                        Society (\citealp{AboutSWEBOK}; see \Cref{metas}).}, and
          \end{enumerate}
...
\begin{figure}[htbp!]
    ...
    \begin{tikzpicture}
        ...
        % ISO/IEC
        \node[above] at (0, 0.23*\radius) {\parbox{3.25cm}{\centering
                \citealp{ISO_IEC2023a,ISO_IEC2023b,ISO_IEC2018,ISO_IEC2015,
                    ISO_IEC2014a,ISO_IEC2014b,ISO_IEC2014c,ISO_IEC2011,ISO_IEC2005}}};
        % ISO/IEEE
        \node[above] at ( 0.9*\offset, -0.9*\offset) {---};
        % IEC/IEEE
        \node[above] at (-0.9*\offset, -0.9*\offset) {---};

        % ISO
        \node[above] at (-1.3*\offset, 0.25) {\parbox{2cm}{\centering
                \citealp{ISO2022,ISO2015}}};
        % IEC
        \node[above] at ( 1.3*\offset, 0.25) {---};
        % IEEE
        \node[above] at (0, -\spread-1.4*\radius) {\parbox{4.5cm}{\centering
                \citealp{IEEE2024,IEEE2012,IEEE2009}}};
...
one kind of flaw, the next logical step is then to detect more. We detect the
following classes of flaws that we later discuss in \Cref{cats,pars}:

\begin{itemize}
    \item \multiCatIntro*{}.
    \item \reflexParChdFlaw{}; a case of this with approach ``I'' would result
          in output similar to \Cref{fig:selfExampleGraph}.
    \item Pairs of approaches with a synonym relation \emph{and} a parent-child
          relation as described in \Cref{parSynDef}; a case of this with
          approaches ``J'' and ``K'' would result in output similar to
...
% test approach node in these visualizations. This process .% \Cref{selfPars}
% \phantomsection{}\label{autoParSyn} We use a similar process to detect.
% To find these pairs, we build a dictionary of
...
% We present these two. \Cref{tab:parSyns,infParSyns}
...
should not be double counted as both! When presenting % \thesisissuetodo{83,176}
...
\seeSrcCode{f009b67}{scripts/flawCounter.py}{63}{85}.
...
% written by the same standards organizations (\noHyperIEEE{}), we count
...
We also parse implicit information following
the same rules given in \Cref{auto-flaw-detect-explicitness} for
automatically detecting flaws.
...
We also generate more involved macros for flaw counts and sections. We count
flaws based on their manifestation and domain, explicitness, and source tier,
then store these data in corresponding files that we read in and assign to
macros. For example, \macro{srcCount} is simply an alias for
\macro[15]{totalFlawDmnBrkdwn} (which is equivalent to
\macro[13]{totalFlawMnfstBrkdwn}). Similarly, we use scripts to generate macros
for flaw manifestations, flaw domains, and source tiers as shown in
\Cref{tab:macrosSections}. Macros for the latter take an integer input as
follows to access either:
\begin{enumerate}
    \item the source tier's name,
    \item the list of sources in the tier, or
...
% calculated values, we use scripts to generate macros for flaw manifestations,
