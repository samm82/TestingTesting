...
\begin{displayquote}
    \texttt{\% Flaw count (SYN, SEM): \{A1\} \{A2\} \dots{} | \{B1\} \{B2\}
        \dots{} | \{C1\} \dots}
\end{displayquote}
\texttt{SYN} and \texttt{SEM} are placeholders for the ``keys'' given in
\Cref{tab:flawMnfstDefs,tab:flawDmnDefs} that we use to identify
a flaw's syntax and semantics (see \Cref{flaws}). We omit these keys from
