...
\refHelper \citet[p.~7]{KanerEtAl2011}
\multiAuthHelper{give} the example of complete testing, which may require
...
robustness, the same should apply to its supporting literature!
% \ifnotpaper
...
process rather than the test approaches themselves:
...
\begin{itemize}
    \item \textbf{Error:} a ``human action that produces an incorrect result''
          \ifnotpaper (\citealp[p.~165]{IEEE2017}; \citeyear[p.~128]{IEEE2010};
              \citealp[p.~399]{vanVliet2000}; similar in \citealp[p.~36]{IEEE2024})\else
              \cite[p.~128]{IEEE2010}, \cite[p.~165]{IEEE2017},
              \cite[p.~399]{vanVliet2000} (similar in \citealp[p.~36]{IEEE2024})\fi.
    ...
    \item \textbf{Fault:} ``an incorrect step, process, or data definition in a
          computer program'' \ifnotpaper (\citealp[p.~140]{IEEE2010}; similar
              in \citealp[p.~36]{IEEE2024}) \else \cite[p.~140]{IEEE2010}
              (similar in \citealp[p.~36]{IEEE2024}) \fi inserted when a
          developer makes an error \ifnotpaper (p.~36; \citealp[pp.~128, 140]{IEEE2010};
              \citealp[pp.~399\==400]{vanVliet2000})\else \cite[pp.~128, 140]{IEEE2010},
              \cite[pp.~399\==400]{vanVliet2000}, \cite[p.~36]{IEEE2024}\fi.
    \item \textbf{Failure:} the inability of a system ``to perform a required
          ...
          ``externally visible'' \ifnotpaper (\citealp[p.~7]{IEEE2019a}; similar
              in \citealp[pp.~15, 37]{IEEE2024}; \citealp[p.~5\=/3]{SWEBOK2025};
              \citealp[p.~12]{Lyu1996}; \citealp[p.~400]{vanVliet2000})
          \else \cite[p.~7]{IEEE2019a} (similar in \cite[p.~400]{vanVliet2000},
              \cite[pp.~15, 37]{IEEE2024}, \cite[p.~5\=/3]{SWEBOK2025},
              \cite[p.~12]{Lyu1996}) \fi and caused by a fault \ifnotpaper
              (\citealp[p.~12\=/3]{SWEBOK2025}; \citealp[p.~13]{Lyu1996};
              \citealp[p.~400]{vanVliet2000})\else \cite[p.~400]{vanVliet2000},
              \cite[p.~12\=/3]{SWEBOK2025}, \cite[p.~13]{Lyu1996}\fi.
    \item \textbf{Defect:} ``an imperfection or deficiency in a project
          ...
          \ifnotpaper (\citealp[p.~96]{IEEE2010}; \citealp[p.~536]{PMBOK2013})\else
              \cite[p.~96]{IEEE2010}, \cite[p.~536]{PMBOK2013}\fi.
\end{itemize}
These distinctions are often important, but the term ``defect'' ``may refer to
any or all of error, fault, or failure'' \citep[p.~36]{IEEE2024}, making the
term ``overloaded with too many meanings, as
engineers and others use the word to refer to all different types of
anomalies'' \citep[p.~12\=/3]{SWEBOK2025}. Software testers
...
literature. % This flaw is automatically tracked as a multiCat flaw
\citeauthor{IEEE2022} \citeyearpar[Fig.~2]{IEEE2022} categorize
experience-based testing as both a test design technique and a test practice in
the same figure! \tourFlaw*{} \loadFlaw{} \alphaFlaw{}
...
tests code itself%\thesisissuetodo{22}
...
\input{build/methodOverviewIntro} We follow the procedure laid out in
\Cref{methodology} and use these \acfp{rq} as a guide:
\begin{enumerate}
    \item \rqatext{}
    \item \rqbtext{}
          % \item \rqctext{}
...
Despite the amount of well-understood and organized knowledge, we find the
literature to be quite flawed (\Cref{flaws}), reinforcing the need for a proper
taxonomy! % We then provide some potential solutions covering some of these
% flaws (\Cref{app-recs}).
\ifnotpaper
    % TODO: Update the paper Conclusion and include a pointer to it here
    We then describe the threats to the validity of our work
    (\Cref{threats}), outline potential next steps for future researchers
    (\Cref{future-work}), and summarize our findings (\Cref{conclusion}).
